...
from app.services.loi_service import generate_loi_from_analysis, loi_service
...
async def generate_loi(request: GenerateLOIRequest, current_user: dict | None = Depends(get_current_user_optional)):
    """
    ...
    try:
        ...
        logger.warning(f"LOI validation error: {e!s}")
        raise HTTPException(status_code=400, detail=f"Invalid LOI request: {e!s}")
    except Exception as e:
        logger.error(f"Failed to generate LOI: {e!s}")
        raise HTTPException(status_code=500, detail=f"Failed to generate LOI: {e!s}")


@router.post("/generate/pdf")
async def generate_loi_pdf(request: GenerateLOIRequest, current_user: dict | None = Depends(get_current_user_optional)):
    """
    ...
    try:
        ...
        logger.warning(f"PDF LOI validation error: {e!s}")
        raise HTTPException(status_code=400, detail=f"Invalid request: {e!s}")
    except Exception as e:
        logger.error(f"Failed to generate PDF LOI: {e!s}")
...
async def generate_loi_from_wholesale_analysis(
    ...
    try:
        ...
        logger.warning(f"LOI from analysis validation error: {e!s}")
        raise HTTPException(status_code=400, detail=f"Invalid analysis data: {e!s}")
    except Exception as e:
        logger.error(f"Failed to generate LOI from analysis: {e!s}")
        raise HTTPException(status_code=500, detail=f"Failed to generate LOI: {e!s}")
...
async def quick_generate_loi(
    ...
    try:
        request = GenerateLOIRequest(
            buyer=BuyerInfo(name=buyer_name, company=buyer_company, email=buyer_email, phone=buyer_phone),
            property_info=PropertyInfo(
                address=property_address, city=property_city, state=property_state, zip_code=property_zip
            ),
            terms=LOITerms(
        ...
        logger.warning(f"Quick LOI validation error: {e!s}")
        raise HTTPException(status_code=400, detail=f"Invalid request: {e!s}")
    except Exception as e:
        logger.error(f"Quick LOI generation failed: {e!s}")
        raise HTTPException(status_code=500, detail=f"Failed to generate LOI: {e!s}")
...
async def get_user_preferences(current_user: dict | None = Depends(get_current_user_optional)):
    """
...
async def save_user_preferences(
    preferences: LOIUserPreferences, current_user: dict | None = Depends(get_current_user_optional)
):
    """
    ...
    logger.info("Saving LOI preferences for user")
    return preferences
...
async def get_loi_history(
    limit: int = 20, offset: int = 0, current_user: dict | None = Depends(get_current_user_optional)
):
    """
...
async def get_loi_by_id(loi_id: str, current_user: dict | None = Depends(get_current_user_optional)):
    """
    ...
    raise HTTPException(status_code=404, detail=f"LOI {loi_id} not found")
