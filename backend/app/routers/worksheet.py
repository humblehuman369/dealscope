...
class STRWorksheetInput(BaseModel):
    ...
    cleaning_fee_revenue: float | None = None
    avg_booking_length: int | None = None
...
async def calculate_ltr_worksheet(input_data: LTRWorksheetInput, db: DbSession):
    ...
    try:
        ...
        pm = (
            input_data.property_management_pct
            if input_data.property_management_pct is not None
            else o.property_management_pct
        )
        ...
        result = calculate_ltr(
            ...
            interest_rate=ir,
            loan_term_years=lt,
            closing_costs_pct=input_data.closing_costs / input_data.purchase_price
            if input_data.closing_costs > 0
            else f.closing_costs_pct,
            vacancy_rate=vr,
...
async def calculate_str_worksheet(input_data: STRWorksheetInput, db: DbSession):
    ...
    try:
        ...
        pmp = (
            input_data.property_management_pct
            if input_data.property_management_pct is not None
            else s.str_management_pct
        )
        cpt = (
            input_data.cleaning_cost_per_turn
            if input_data.cleaning_cost_per_turn is not None
            else s.cleaning_cost_per_turnover
        )
...
async def calculate_brrrr_worksheet(input_data: BRRRRWorksheetInput, db: DbSession):
    ...
    try:
        ...
        purchase_costs_pct = (
            ...
            else a.financing.closing_costs_pct
        )
        down_payment_pct = (
            (1 - (input_data.loan_to_cost_pct / 100))
            if input_data.loan_to_cost_pct is not None
            else input_data.down_payment_pct
        )
        ...
        loan_to_cost = (
            ...
            if (input_data.purchase_price + input_data.rehab_costs) > 0
            else 0
        )
...
async def calculate_househack_worksheet(input_data: HouseHackWorksheetInput):
    ...
    try:
        ...
        maintenance_monthly = (
            ...
            if input_data.maintenance_monthly == 0
            else input_data.maintenance_monthly
        )
        ...
        income_value = input_data.purchase_price + (
        ...
        return {
            ...
            "coc_return": (full_rental_cash_flow / result["total_cash_required"] * 100)
            if result["total_cash_required"] > 0
            else 0,
            "deal_score": min(
                100,
                max(
