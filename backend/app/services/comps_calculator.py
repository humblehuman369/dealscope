...
class CompsCalculator:
    ...
    def __init__(self, factors: Optional[AdjustmentFactors] = None):
    ...
    def calculate_weighted_hybrid_value(
        ...
        weighted_adjusted_avg = sum(
            ca.adjusted_price * ca.weight for ca in comp_adjustments
        )
        
        # Method 2: Weighted price-per-sqft
        weighted_ppsf = sum(
            ca.price_per_sqft * ca.weight for ca in comp_adjustments
        )
    ...
    def calculate_rent_value(
        ...
        subject: Dict[str, Any],
        selected_comps: List[Dict[str, Any]],
        improvement_premium_pct: float = 0.10
    ) -> Dict[str, Any]:
        """
        ...
        weighted_adjusted_rent = sum(
            ca["adjusted_rent"] * ca["weight"] for ca in comp_adjustments
        )
        weighted_rent_per_sqft = sum(
            ca["rent_per_sqft"] * ca["weight"] for ca in comp_adjustments
        )
