...
class SearchHistoryService:
    ...
    async def get_user_history(
        ...
        """
        query = select(SearchHistory).where(SearchHistory.user_id == UUID(user_id))

        if successful_only:
            query = query.where(SearchHistory.was_successful)
    ...
    async def get_history_count(
        ...
        query = select(func.count()).select_from(SearchHistory).where(SearchHistory.user_id == UUID(user_id))
    ...
    async def get_stats(
        ...
        # Total searches
        total_query = select(func.count()).select_from(SearchHistory).where(SearchHistory.user_id == user_uuid)
        ...
        # Top markets (states)
        markets_query = (
            select(SearchHistory.address_state, func.count().label("count"))
            .where(
                and_(
                    SearchHistory.user_id == user_uuid,
            ...
            .group_by(SearchHistory.address_state)
            .order_by(desc(func.count()))
            .limit(5)
        )

        markets_result = await db.execute(markets_query)
        top_markets = [{"state": row.address_state, "count": row.count} for row in markets_result.all()]
    ...
    async def clear_history(
        ...
        query = delete(SearchHistory).where(SearchHistory.user_id == UUID(user_id))
