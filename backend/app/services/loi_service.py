...
class LOIService:
    ...
    def _init_styles(self):
        ...
        self.styles.add(
            ParagraphStyle(name="LOIDate", parent=self.styles["Normal"], fontSize=10, alignment=TA_RIGHT, spaceAfter=20)
        )

        self.styles.add(ParagraphStyle(name="LOISignature", parent=self.styles["Normal"], fontSize=10, spaceBefore=30))
    ...
    def _generate_text(self, request: GenerateLOIRequest, loi_id: str, expiration_date: date) -> str:
    ...
    def _generate_html(self, request: GenerateLOIRequest, loi_id: str, expiration_date: date) -> str:
...
"""
    ...
    def _generate_pdf(self, request: GenerateLOIRequest, loi_id: str, expiration_date: date) -> bytes:
        ...
        # Title
        story.append(Paragraph("LETTER OF INTENT TO PURCHASE REAL PROPERTY", self.styles["LOITitle"]))
        story.append(Paragraph(f"Reference: LOI-{loi_id}", self.styles["LOISubtitle"]))
        ...
        for contingency in terms.contingencies:
            ...
            if contingency == ContingencyType.INSPECTION:
                cont_text += (
                    f"<br/><font size=9>{self.INSPECTION_CLAUSE.format(days=terms.inspection_period_days)}</font>"
                )
        ...
        story.append(
            Paragraph(
                "The undersigned acknowledges receipt of this Letter of Intent and agrees to the terms outlined above:",
                self.styles["LOIBody"],
            )
        )
