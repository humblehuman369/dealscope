...
from .api_clients import RentCastClient
from .calculators import calculate_seller_motivation, extract_condition_keywords
from .data_normalizer import DealGapIQNormalizer, NormalizedProperty
from .zillow_client import ZillowAPIResponse, create_zillow_client
...
class UnifiedPropertyService:
    ...
    async def get_property(self, address: str, include_zillow_photos: bool = False) -> dict[str, Any]:
        """
        ...
        rentcast_data, zillow_data = await asyncio.gather(rentcast_task, zillow_task, return_exceptions=True)
        ...
        return {
            ...
            "raw": {"rentcast": rentcast_data, "zillow": self._zillow_responses_to_dict(zillow_data)},
    ...
    async def _fetch_all_rentcast(self, address: str) -> dict[str, Any]:
        ...
        results = {"api": "RentCast", "address": address, "fetched_at": datetime.now(UTC).isoformat(), "endpoints": {}}
        ...
        for name, response in zip(task_names, responses, strict=False):
            if isinstance(response, Exception):
                results["endpoints"][name] = {"status_code": None, "data": None, "error": str(response)}
            else:
    ...
    async def _fetch_all_zillow(self, address: str, include_photos: bool = False) -> dict[str, Any]:
    ...
    def _calculate_investment_metrics(self, prop: NormalizedProperty) -> dict[str, Any]:
        ...
        if rent and annual_tax:
            ...
            if value:
                ...
                if total_cash_in > 0:
                    metrics["estimated_cash_on_cash_roi"] = round((annual_cash_flow / total_cash_in) * 100, 2)
        ...
        if prop.market_avg_price_sqft and sqft and value:
            ...
            metrics["vs_market_value_pct"] = round(((value - expected_value) / expected_value) * 100, 2)
    ...
    def _zillow_responses_to_dict(self, data: dict | None) -> dict | None:
    ...
    def _summarize_provenance(self, prop: NormalizedProperty) -> dict[str, Any]:
        ...
        for _field, prov in prop.provenance.items():
        ...
        return {"by_source": sources, "by_confidence": confidence, "total_fields": len(prop.provenance)}

    async def export_to_csv(self, address: str, output_path: str, include_raw: bool = False) -> str:
        """
        ...
        rows.append(["Generated", datetime.now(UTC).isoformat(), ""])
        ...
        for key in ["property_type", "bedrooms", "bathrooms", "square_footage", "lot_size", "year_built", "stories"]:
        ...
        rows.append(
            [
                "Current AVM Estimate",
                f"${normalized.get('current_value_avm') or 0:,.0f}",
        ...
        rows.append(
            [
                "RentCast Estimate",
                f"${normalized.get('monthly_rent_estimate') or 0:,.0f}",
    ...
    async def export_to_json(self, address: str, output_path: str, include_raw: bool = True) -> str:
        """
...
def create_unified_service(
    rentcast_api_key: str, rentcast_url: str, axesso_api_key: str, axesso_url: str = "https://api.axesso.de/zil"
