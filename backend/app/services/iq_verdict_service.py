...
def _calculate_ltr_strategy(
    ...
    # adds closing + rehab. May be negative when financing exceeds purchase + costs.
    cash_equity = max(0.0, price - loan_amount - sc)
    total_cash = price + closing_costs + rehab_cost - loan_amount - sc
...
def compute_iq_verdict(
    ...
    # Deal Gap measures discount from market price to Target Buy price.
    # Positive = discount needed (list > target buy), zero = deal works at market.
