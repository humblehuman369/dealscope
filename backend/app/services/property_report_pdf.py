...
def _ensure_weasyprint():
    ...
    try:
        ...
        WEASYPRINT_AVAILABLE = False
        logger.error(f"WeasyPrint import FAILED — PDF report generation disabled. Error: {type(exc).__name__}: {exc}")
        raise
...
{self._page_sensitivity()}
    ...
    def _page_cover(self) -> str:
        ...
        prop.lot_size / 43560 if prop.lot_size > 0 else 0
  ...
  {self._page_footer()}
    ...
    def _page_financing(self) -> str:
        ...
        steps = [
            ...
            generate_step_item(
                3, "Monthly Payment", _fmt_money(fin.monthly_payment), "Principal and interest payment", self.theme
            ),
            generate_step_item(
                4,
                "Closing Costs",
    ...
    def _page_income(self) -> str:
        ...
        steps = [
            generate_step_item(
                ...
                _fmt_money(inc.annual_gross_rent),
                f"Based on RentCast estimate of {_fmt_money(inc.monthly_rent)}/month",
                self.theme,
            ),
            generate_step_item(
                2,
                f"Less Vacancy Allowance ({inc.vacancy_percent:.0f}%)",
                f"-{_fmt_money(inc.vacancy_allowance)}",
                f"Conservative {inc.vacancy_percent:.0f}% vacancy rate",
                self.theme,
            ),
            generate_step_item(
                3,
                "Effective Gross Income",
                _fmt_money(inc.effective_gross_income),
                "Net rental income post-vacancy",
                self.theme,
            ),
            generate_step_item(
                4,
                "Total Operating Expenses",
                f"-{_fmt_money(exp.total_operating_expenses)}",
                "Taxes, insurance, maintenance & operational costs",
                self.theme,
            ),
            generate_step_item(
                5,
                "Net Operating Income",
                _fmt_money(m.net_operating_income),
            ...
            generate_step_item(
                6,
                "Annual Debt Service",
                f"-{_fmt_money(m.annual_debt_service)}",
                "P&I payments on mortgage",
                self.theme,
            ),
            generate_step_item(
                7,
                "Pre-Tax Cash Flow",
                _sign_money(m.annual_cash_flow),
  ...
  {self._page_footer()}
    ...
    def _page_expenses(self) -> str:
        ...
        donut = generate_donut_chart(
            ...
            height=200,
            inner_label="Total",
            inner_value=_fmt_money(exp.total_operating_expenses),
        )
  ...
  {self._page_footer()}
    ...
    def _page_metrics(self) -> str:
        ...
        cards = [
            ...
            generate_metric_card(
                "Cash-on-Cash",
                _fmt_pct(m.cash_on_cash_return),
                coc_assess,
...
<div class="page">
  ...
  {self._page_footer()}
    ...
    def _page_deal_score(self) -> str:
        ...
        score_ring = generate_score_ring(ds.score, 100, self.theme, size=140, grade=ds.grade)

        # Return factor bars
        cap_bar = generate_gauge_bar(m.cap_rate, 12.0, 6.0, self.theme, width=180)
        coc_bar = generate_gauge_bar(max(0, m.cash_on_cash_return), 20.0, 8.0, self.theme, width=180)
  ...
  {self._page_footer()}
    ...
    def _page_projections(self) -> str:
        ...
        area_chart = generate_area_chart(
            labels, prop_values, equity_values, "Property Value", "Equity Position", self.theme, width=520, height=220
        )
    ...
    def _page_exit(self) -> str:
        ...
        steps = [
            generate_step_item(
                ...
                _fmt_money(e.projected_sale_price),
            ...
            generate_step_item(
                2,
                "Sale Deductions",
                f"-{_fmt_money(e.total_sale_costs)}",
            ...
            generate_step_item(
                3,
                "Loan Payoff",
                f"-{_fmt_money(e.remaining_loan_balance)}",
            ...
            generate_step_item(
                4,
                "Pre-Tax Proceeds",
                _fmt_money(e.net_sale_proceeds),
