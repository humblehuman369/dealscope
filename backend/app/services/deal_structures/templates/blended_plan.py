...
def solve(
    ...
    *,
    ...
    pitch = (
        f"Here is what would make this work for me: "
        f"price at {fmt_money_precise(new_price)}, "
        f"you carry {fmt_money_precise(chosen_second)} as a 0% second for "
        f"{DEFAULT_BALLOON_YEARS} years, "
        f"and I will verify market rent at ${round(new_rent):,}/mo with comps. "
        f"None of those asks is large on its own — together they make the deal work."
    )
