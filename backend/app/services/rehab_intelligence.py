...
class RehabEstimate:
    ...
    # Holding costs
    holding_costs: HoldingCosts | None = None
...
class RehabIntelligence:
    ...
    def __init__(
        ...
        has_pool: bool = False,
        roof_type: str | None = None,  # "shingle", "tile", "metal", "flat"
        stories: int = 1,
        garage_spaces: int = 0,
        lot_sqft: int | None = None,
        hoa_monthly: float = 0,
        # Permit history (items recently replaced)
        recent_permits: list[str] | None = None,
        # Custom overrides
        custom_location_factor: float | None = None,
    ):
        self.sq_ft = sq_ft
        ...
        if custom_location_factor:
            ...
            loc_data = LOCATION_FACTORS.get(self.zip_code, LOCATION_FACTORS["default"])
    ...
    def calculate(
        ...
        if wet_factor >= 0.8:
            bath_cost = (full_baths * self._apply_multipliers(self.costs.full_bath_remodel)) + (
                half_baths * self._apply_multipliers(self.costs.half_bath_remodel)
            )
        else:
            bath_cost = (full_baths * self._apply_multipliers(self.costs.full_bath_cosmetic) * wet_factor) + (
        ...
        if self.asset_class == AssetClass.ULTRA_LUXURY:
            ...
            flooring_cost = (
                flooring_sqft * ((self.costs.flooring_tile + self.costs.flooring_hardwood) / 2) * self.location_factor
            )
        else:
        ...
        if self.property_age > CAPEX_AGE_THRESHOLDS["water_heater"] and "water_heater" not in self.recent_permits:
            wh_cost = (
                self.costs.water_heater_tankless
                if self.asset_class != AssetClass.STANDARD
                else self.costs.water_heater_tank
            )
        ...
        if include_holding_costs:
            holding_costs = self._calculate_holding_costs(breakdown.total, holding_loan_rate, holding_ltv)
    ...
    def generate_line_items(self) -> list[dict[str, Any]]:
        ...
        tier = (
            ...
            if self.asset_class == AssetClass.LUXURY
            else "low"
        )
