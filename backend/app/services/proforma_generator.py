...
Airbnb and VRBO. Revenue is driven by Average Daily Rate (ADR) x Occupancy Rate.
...
Rule: MAO = ARV x 70% - Rehab Costs - Your Fee. Lowest capital requirement of
    ...
    try:
        if strategy == "ltr":
            ...
            return calculate_brrrr(
                ...
                arv=arv or purchase_price * 1.25,
                monthly_rent_post_rehab=monthly_rent,
                property_taxes_annual=property_taxes,
            ...
            return calculate_house_hack(
                purchase_price=purchase_price,
                monthly_rent_per_room=monthly_rent / max(bedrooms, 2),
            ...
            return calculate_wholesale(
                arv=arv or purchase_price * 1.3,
                estimated_rehab_costs=rehab_cost,
            )
        else:
            return {}
...
def get_yearly_amortization(schedule: list[AmortizationRow], year: int) -> tuple[float, float, float]:
...
def calculate_investment_returns(
    initial_investment: float, annual_cash_flows: list[float], exit_proceeds: float
) -> InvestmentReturns:
    ...
    average_annual_return = (
        (total_cash_flows / len(annual_cash_flows)) / initial_investment * 100
        if annual_cash_flows and initial_investment > 0
        else 0
    )
...
def calculate_sensitivity_for_interest_rate(
    ...
    maintenance = annual_gross_rent * a.operating.maintenance_pct
    utilities = a.operating.utilities_monthly * 12
    total_op_ex = property_taxes + insurance + management + maintenance + utilities + annual_gross_rent * 0.05
...
def generate_full_sensitivity_analysis(
    ...
    for pct in percent_changes:
        ...
        results = calculate_sensitivity_for_purchase_price(
            base_params, new_price, hold_period_years, marginal_tax_rate, capital_gains_tax_rate, assumptions=a
        )
        purchase_price_scenarios.append(
            SensitivityScenario(
    ...
    for delta in rate_changes:
        ...
        results = calculate_sensitivity_for_interest_rate(base_params, new_rate, hold_period_years, assumptions=a)
        interest_rate_scenarios.append(
            SensitivityScenario(
    ...
    for pct in percent_changes:
        ...
        results = calculate_sensitivity_for_rent(base_params, new_rent, hold_period_years, assumptions=a)
    ...
    for vac in vacancy_changes:
        ...
        utilities = a.operating.utilities_monthly * 12
        total_op_ex = (
            base_params["property_taxes"] + insurance + management + maintenance + utilities + annual_gross_rent * 0.05
        )

        noi = effective_income - total_op_ex
        annual_cash_flow = noi - base_params["monthly_mortgage"] * 12
        cash_on_cash = (
            (annual_cash_flow / base_params["total_cash_required"] * 100)
            if base_params["total_cash_required"] > 0
            else 0
        )

        vacancy_scenarios.append(
            SensitivityScenario(
                variable="vacancy",
...
async def generate_proforma_data(
    ...
    down_payment_pct_override: float | None = None,
    property_taxes_override: float | None = None,
    ...
    annual_gross_rent = monthly_rent * 12
    property_taxes = (
        property_taxes_override or (market.property_taxes_annual if market else None) or (purchase_price * 0.01)
    )
    ...
    break_even_occupancy = (
        ((total_operating_expenses + annual_debt_service) / annual_gross_rent * 100) if annual_gross_rent > 0 else 0
    )
    ...
    return FinancialProforma(
        ...
        expenses=ExpenseDetails(
            ...
            expense_ratio=(total_operating_expenses / effective_gross_income * 100)
