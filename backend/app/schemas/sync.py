...
class SyncPullRequest(BaseModel):
    ...
    since: Optional[int] = Field(
        None,
        description="Unix timestamp (seconds). Only return changes after this time. If null, return all data."
    )
    tables: List[str] = Field(
        default=["scanned_properties", "portfolio_properties", "settings"],
...
class SyncRecord(BaseModel):
    ...
    data: Optional[Dict[str, Any]] = Field(None, description="Record data (null for deletes)")
...
class SyncPullResponse(BaseModel):
    ...
    next_since: Optional[int] = Field(None, description="Use this as 'since' for next request if has_more is true")
...
class SyncPushResult(BaseModel):
    ...
    server_updated_at: Optional[int] = None
    conflict: bool = False
    resolution: Optional[str] = None  # "local_wins" or "server_wins"
