...
class Settings(BaseSettings):
    ...
    DATABASE_URL: str = "postgresql+asyncpg://postgres:postgres@localhost:5432/dealgapiq"
    ...
    REDFIN_API_KEY: str = ""
    REDFIN_URL: str = "https://redfin-base.p.rapidapi.com"  # legacy; prefer RAPIDAPI_HOST
    RAPIDAPI_HOST: str = "redfin-com-data.p.rapidapi.com"  # Redfin API host (X-Rapidapi-Host)
    ...
    # domains (e.g. dealgapiq.com + dealscope-production.up.railway.app).
    # CSRF middleware provides equivalent protection.
    COOKIE_SAMESITE: str = "none"  # lax, strict, or none
    COOKIE_DOMAIN: str | None = None  # Set for cross-subdomain cookies
    ...
    S3_BUCKET_NAME: str = "dealgapiq-documents"
    S3_ENDPOINT_URL: str | None = None  # For R2: https://{account_id}.r2.cloudflarestorage.com

    # Cloudflare R2 (S3-compatible)
    R2_ACCOUNT_ID: str = ""
    R2_ACCESS_KEY_ID: str = ""
    R2_SECRET_ACCESS_KEY: str = ""
    R2_BUCKET_NAME: str = "dealgapiq-documents"
    ...
    EXPO_ACCESS_TOKEN: str = (
        ""  # Optional — improves throughput; get from https://expo.dev/accounts/[account]/settings/access-tokens
    )
    ...
    @classmethod
    def normalize_frontend_url(cls, v: str | None) -> str:
...
def validate_settings(settings: Settings) -> None:
    ...
    if not settings.SECRET_KEY or len(settings.SECRET_KEY) < 32:
        if settings.is_production:
            errors.append("SECRET_KEY must be set and at least 32 characters. Generate with: openssl rand -hex 32")
        else:
            warnings.warn(
    ...
    if settings.is_production:
        ...
        for key_name, key_value in _optional_keys:
            if not key_value:
                warnings.warn(f"{key_name} not set — related features will be unavailable", stacklevel=2)
