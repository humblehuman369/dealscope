/**
 * Record live sales-demo scenes against production (or a local base URL).
 *
 *   npx tsx scripts/walkthrough-video/record.ts
 *   npx tsx scripts/walkthrough-video/record.ts --base-url https://dealgapiq.com
 *   npx tsx scripts/walkthrough-video/record.ts --headed
 *   npx tsx scripts/walkthrough-video/record.ts --scene 03-verdict
 */
import { chromium, type Browser, type BrowserContext, type Page } from 'playwright';
import * as fs from 'fs';
import * as path from 'path';
import { execSync } from 'child_process';
import { loadWalkthroughEnv, loadScenes, SCENES_DIR, type Scene } from './loadEnv';
...
async function waitForVerdict(page: Page) {
...
/** Record a scene into a temporary context with recordVideo, then convert to mp4. */
async function recordScene(
  ...
  try {
    ...
    if (elapsed < targetSec) {
      await sleep((targetSec - elapsed) * 1000);
    }
  } finally {
    await page.close();
    await context.close();
  ...
  execSync(
    `ffmpeg -y -i "${src}" -c:v libx264 -pix_fmt yuv420p -an "${dest}"`,
    { stdio: 'pipe' },
  );
  fs.rmSync(sceneDir, { recursive: true, force: true });
...
async function sceneHook(page: Page) {
  await page.goto(`${BASE_URL}/`, { waitUntil: 'domcontentloaded' });
  await dismissChrome(page);
  await sleep(2000);
  // Flash entry affordances
...
async function sceneAnalyze(page: Page) {
  await page.goto(`${BASE_URL}/`, { waitUntil: 'domcontentloaded' });
  await dismissChrome(page);
  await sleep(800);
  // Prefer direct discovery navigation after a brief search UI beat
  const search = page.locator('input[placeholder*="address" i], input[placeholder*="search" i]').first();
  ...
  const encoded = encodeURIComponent(DEMO_ADDRESS);
  await page.goto(`${BASE_URL}/analyzing?address=${encoded}`, { waitUntil: 'domcontentloaded' }).catch(async () => {
    await page.goto(`${BASE_URL}/discovery?address=${encoded}`, { waitUntil: 'domcontentloaded' });
  });
  await sleep(4000);
  await page.goto(`${BASE_URL}/discovery?address=${encoded}`, { waitUntil: 'domcontentloaded' });
...
async function sceneVerdict(page: Page) {
...
async function sceneTrustStrategies(page: Page) {
  await waitForVerdict(page);
  await dismissChrome(page);
  // Try IQ selector
  const iq = page.locator('button:has-text("IQ"), [aria-label*="estimate" i], [class*="IQEstimate"]').first();
  ...
  const strat = page.locator('text=Long-term, text=LTR, text=Wholesale, text=House Hack').first();
...
async function sceneFourPaths(page: Page) {
  await waitForVerdict(page);
  await dismissChrome(page);
  const paths = page.locator('text=Four Paths, text=Blended Plan, text=Path 1, text=structure').first();
  ...
  await sleep(3000);
  const card = page.locator('button:has-text("Path"), [class*="path"], button:has-text("Blended")').first();
...
async function scenePitch(page: Page) {
  await waitForVerdict(page);
  await dismissChrome(page);
  await page.evaluate(() => window.scrollBy({ top: 1600, behavior: 'smooth' }));
  await sleep(1500);
  const pitch = page
    .locator('button:has-text("Pitch"), button:has-text("Script"), a:has-text("Pitch Script"), button:has-text("View Script")')
    .first();
  if (await pitch.isVisible({ timeout: 5000 }).catch(() => false)) {
    ...
    // Fallback: stay on Four Paths region
    await page.evaluate(() => window.scrollBy({ top: 400, behavior: 'smooth' }));
    await sleep(6000);
  }
  await sleep(2000);
...
async function sceneDealMaker(page: Page) {
  const encoded = encodeURIComponent(DEMO_ADDRESS);
  const urls = [
...
async function main() {
  ...
  // Prime verdict cache while warm session is open
  if (ok) {
  ...
  for (const scene of scenes) {
    ...
    // Prefer WAV duration when available
