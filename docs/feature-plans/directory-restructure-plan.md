...
**implemented**. Stage 3 is **partial**: the `directory_service_area` table and the
lender state backfill shipped; the buyer county backfill is blocked on `geo_city`
(see Stage 3). Stage 4 and the rest of Stage 5 remain proposed.
...
### Stage 3 — `directory_service_area` and real county matching — PARTIAL 2026-07-29

**Goal:** one indexed answer to "who covers this location?" for both directories.

**Shipped:** the table, and the lender state backfill (7,093 rows: 7,014 state +
79 nationwide). A state lookup through the table is exactly equivalent to the old
`states_served` array filter — asserted across all 51 states, in a test, not just
once by hand.

**Not shipped: the buyer county backfill. It is blocked on missing data**, which
measurement established rather than assumption. Of 9,871 `cash_buyers.coverage[]`
strings, only **59.6% can be resolved** today:

| Outcome | Count | Share |
|---|---:|---:|
| resolved to a county | 5,541 | 56.1% |
| state-wide markers (`All of CA`, `58 CA Counties`) | 344 | 3.5% |
| **unmatched — almost entirely city names** | **3,504** | **35.5%** |
| buyer has no state, so the lookup cannot be scoped | 386 | 3.9% |
| ambiguous (the known Baltimore / St. Louis / Virginia cases) | 96 | 1.0% |

The unmatched third is not noise: it is San Antonio, Fort Worth, Orlando, Tampa,
Atlanta, Charlotte, Nashville, Chicago, Seattle, Cleveland — the largest markets in
the directory. **`geo_city` is therefore a prerequisite for the buyer half, not the
deferred nicety the geo spec treats it as.** Backfilling only the 59.6% and pointing
the buyer query at the new table would silently drop buyers that today's
`coverage[]` ILIKE search still finds, which is a regression, so the buyer query is
deliberately left on its existing path until coverage is complete.

Full worklist: `docs/geo/coverage-unmatched.csv` (2,135 distinct strings, ranked by
occurrence), regenerated by `scripts/report_buyer_coverage_gaps.py`.
