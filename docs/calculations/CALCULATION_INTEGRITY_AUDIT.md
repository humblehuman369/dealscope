...
| **IQ Verdict Score** | Core Proprietary | `backend/app/services/iq_verdict_service.py` | Bracket interpolation: `INVESTOR_DISCOUNT_BRACKETS` (≤5%→95–88, ≤10%→88–75, …); score = max(5, min(95, interpolate(deal_gap_pct))) | deal_gap_pct | deal_score | correct |
| **Cumulative investor probability** | Core Proprietary + public baselines | `iq_verdict_service._get_cumulative_investor_pct`, `app.core.regions` | Cumulative P(discount ≥ Deal Gap) within `REGIONAL_COHORT_PERCENTAGES[resolve_investor_probability_region(state)]`; uniform within each discount bin. | deal_gap_pct, state | `cumulative_investor_pct`, `deal_probability_score`, `investor_probability_region_label` | correct |
