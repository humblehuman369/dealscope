...
|--------|------------------|------------------|----------------------|
| **Income Value** | `backend/app/core/formulas.estimate_income_value` | `income_value = NOI / WACC` where `WACC = LTV×mortgage_constant + (1−LTV)×required_equity_yield`; NOI = EGI − operating_expenses; `required_equity_yield` default **8%** (`OPERATING.required_equity_yield`). Pure cash: `NOI / required_equity_yield`. | `POST /api/v1/analysis/verdict` → `income_value`; `POST /api/v1/worksheet/deal-score` → `income_value`; Deal Maker, STR/Flip/House Hack worksheets |
