...
|--------|------------------|------------------|----------------------|
| **Income Value** | `backend/app/core/formulas.estimate_income_value` | `income_value = NOI / (LTV × mortgage_constant)`; NOI = EGI − operating_expenses. Pure cash (100% down): `NOI / 0.05` cap-rate floor when denominator is 0. Frontend also blends seller-carry debt constant when seller financing is present. | `POST /api/v1/analysis/verdict` → `income_value`; `POST /api/v1/worksheet/deal-score` → `income_value`; Deal Maker, STR/Flip/House Hack worksheets |
| **Target Buy (Target Price)** | `backend/app/core/formulas.calculate_buy_price` | `buy_price = income_value × (1 - buy_discount_pct)`, capped at `list_price`; default `buy_discount_pct = 0.05` | Verdict → `target_price` / `purchase_price`; Deal Score → `purchase_price` |
...
| **Market Price (off-market)** | `backend/app/core/formulas.compute_market_price` (Property API only) | Listed: Market Price = List Price. Off-market (Property API): Zestimate primary, fallback to AVM, then tax_assessed/0.75. Off-market (Verdict): uses IQ Estimate sent by frontend (no backend override). | Property API `valuations.market_price`; Verdict `list_price` = frontend IQ Estimate cascade |
