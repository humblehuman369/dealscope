 ...
 * structure changed vs the prior baseline. Consumed by `SliderRow` to render
 * a soft accent glow on the matching rows.
 *
 * Default is an empty set, so worksheets used outside the Strategy page (e.g.
 * the standalone DealMaker tab) silently render no glow.
 */
const EMPTY_HIGHLIGHTS: ReadonlySet<string> = new Set()
const WorksheetHighlightContext = createContext<ReadonlySet<string>>(EMPTY_HIGHLIGHTS)
...
type SellerFinancingSlice = Pick<
  LTRDealMakerState,
...
function SellerFinancingPrincipalRow({
  ...
  const minSeller = 0
  const maxSeller = basePrice > 0 ? basePrice * 1.0 : 500000
  return (
    <SliderRow
  ...
  return (
    <>
      <SliderRow
        ...
        value={state.sellerInterestRate * 100}
        displayValue={`${(state.sellerInterestRate * 100).toFixed(2)}%`}
        min={0}
        max={15}
        step={0.25}
        onChange={(v) => up('sellerInterestRate', v / 100)}
        parseInput={(s) => parseFloat(s.replace(/[^0-9.]/g, ''))}
      />
      <SliderRow
        field="sellerTermYears"
        label="Seller Term"
        value={state.sellerTermYears}
        displayValue={`${state.sellerTermYears} years`}
        min={1}
        max={30}
        onChange={(v) => up('sellerTermYears', Math.round(v))}
...
interface SliderRowProps {
  ...
  /**
   * Worksheet state-field name (e.g. `buyPrice`, `downPaymentPercent`,
   * `sellerFinancingAmount`). When this matches an entry in the
   * `WorksheetHighlightContext`, the row renders a soft accent glow to
   * indicate the value was set by an applied Three Paths structure.
   */
  field?: string
}
  ...
  return (
    <>
      ...
      <SliderRow
        ...
        displayValue={fmt(state.buyPrice)}
        min={50000}
        max={2000000}
      ...
      <SliderRow
        ...
        step={1}
        onChange={(ltv) => {
          // Bank Loan is the stored financing input (LTV % of buy price). The down
          // payment is derived from it, so we only write the loan — no feedback through dp%.
      ...
      <SliderRow
        ...
        onChange={(ltv) => {
          ...
          // Pin the current bank loan as the stored input so it stays put; the derived
          // down payment absorbs the seller-financing change.
      ...
      <SliderRow
        ...
        secondaryValue={`${(state.closingCostsPercent * 100).toFixed(1)}%`}
        displayValue={fmt(closingCosts)}
        min={0}
        max={5}
        onChange={(v) => up('closingCostsPercent', v / 100)}
        parseInput={(s) => {
      ...
      <SliderRow
        ...
        displayValue={fmt(state.rehabBudget)}
        min={0}
        max={100000}
      ...
      <SliderRow
        ...
        max={12}
        step={0.25}
        onChange={(v) => up('interestRate', v / 100)}
        parseInput={(s) => parseFloat(s.replace(/[^0-9.]/g, ''))}
      />
      <SliderRow
        field="loanTermYears"
        ...
        min={10}
        max={30}
        onChange={(v) => up('loanTermYears', Math.round(v))}
        parseInput={(s) => parseInt(s.replace(/[^0-9]/g, ''), 10)}
      />
      ...
      <SliderRow
        ...
        displayValue={`${fmt(state.annualPropertyTax)}/yr`}
        min={0}
        max={20000}
        onChange={(v) => up('annualPropertyTax', v)}
      />
      <SliderRow
        label="Insurance"
        value={state.annualInsurance}
        displayValue={`${fmt(state.annualInsurance)}/yr`}
        min={0}
        max={10000}
        onChange={(v) => up('annualInsurance', v)}
      />
      <SliderRow
        label="HOA / Condo Fee"
        value={state.monthlyHoa}
        displayValue={`${fmt(state.monthlyHoa)}/mo`}
        min={0}
        max={1500}
      ...
      <SliderRow
        ...
        secondaryValue={`${(state.capexRate * 100).toFixed(0)}%`}
        displayValue={`${fmt(opex.reserves)}/yr`}
        min={0}
        max={15}
        onChange={(v) => up('capexRate', v / 100)}
        parseInput={(s) => parseFloat(s.replace(/[^0-9.]/g, ''))}
      />
      <SliderRow
        label="Utilities"
        value={state.utilitiesMonthly}
        displayValue={`${fmt(state.utilitiesMonthly)}/mo`}
        min={0}
        max={1000}
        onChange={(v) => up('utilitiesMonthly', v)}
      />
      <SliderRow
        label="Pest Control"
        value={state.pestControlAnnual}
        displayValue={`${fmt(state.pestControlAnnual)}/yr`}
        min={0}
        max={2000}
      ...
      <SliderRow
        ...
        displayValue={fmt(state.monthlyRent)}
        min={500}
        max={10000}
  ...
  return (
    <>
      ...
      <SliderRow
        ...
        displayValue={fmt(state.furnitureSetupCost)}
        min={0}
        max={30000}
      ...
      <SliderRow
        ...
        displayValue={fmt(state.averageDailyRate)}
        min={50}
        max={1000}
        onChange={(v) => up('averageDailyRate', v)}
      />
      <SliderRow
        label="Occupancy"
        ...
        min={30}
        max={95}
        onChange={(v) => up('occupancyRate', v / 100)}
        parseInput={(s) => parseFloat(s.replace(/[^0-9.]/g, ''))}
      />
      <SliderRow
        label="Cleaning Fee"
        value={state.cleaningFeeRevenue}
        displayValue={fmt(state.cleaningFeeRevenue)}
        min={0}
        max={300}
        onChange={(v) => up('cleaningFeeRevenue', v)}
      />
      <SliderRow
        label="Avg Stay"
        ...
        min={1}
        max={30}
        onChange={(v) => up('avgLengthOfStayDays', Math.round(v))}
        parseInput={(s) => parseInt(s.replace(/[^0-9]/g, ''), 10)}
      />
      ...
      <SliderRow
        ...
        min={10}
        max={20}
        onChange={(v) => up('platformFeeRate', v / 100)}
      />
      <SliderRow
        ...
        min={0}
        max={25}
        onChange={(v) => up('strManagementRate', v / 100)}
      />
      <SliderRow
        label="Cleaning/Turn"
        value={state.cleaningCostPerTurnover}
        displayValue={fmt(state.cleaningCostPerTurnover)}
        min={50}
        max={400}
        onChange={(v) => up('cleaningCostPerTurnover', v)}
      />
      <SliderRow
        label="Supplies"
        value={state.suppliesMonthly}
        displayValue={`${fmt(state.suppliesMonthly)}/mo`}
        min={0}
        max={500}
        onChange={(v) => up('suppliesMonthly', v)}
      />
      <SliderRow
        label="Utilities"
        value={state.additionalUtilitiesMonthly}
        displayValue={`${fmt(state.additionalUtilitiesMonthly)}/mo`}
        min={0}
        max={500}
        onChange={(v) => up('additionalUtilitiesMonthly', v)}
      />
      <SliderRow
        label="Property Tax"
        value={state.annualPropertyTax}
  ...
  return (
    <>
      ...
      <SliderRow
        ...
        displayValue={fmt(state.purchasePrice)}
        min={50000}
        max={2000000}
      ...
      <SliderRow
        ...
        max={15}
        step={0.5}
        onChange={(v) => up('hardMoneyRate', v / 100)}
        parseInput={(s) => parseFloat(s.replace(/[^0-9.]/g, ''))}
      />
      ...
      <SliderRow
        ...
        secondaryValue={`${(state.contingencyPct * 100).toFixed(0)}%`}
        displayValue={fmt(state.rehabBudget * state.contingencyPct)}
        min={0}
        max={25}
        onChange={(v) => up('contingencyPct', v / 100)}
        parseInput={(s) => {
          const n = parseFloat(s.replace(/[^0-9.]/g, ''))
          return state.rehabBudget > 0 ? (n / state.rehabBudget) * 100 : 0
        }}
      />
      <SliderRow
        ...
        min={2}
        max={12}
        onChange={(v) => up('holdingPeriodMonths', Math.round(v))}
        parseInput={(s) => parseInt(s.replace(/[^0-9]/g, ''), 10)}
      />
      <SliderRow
        label="Holding Costs"
        value={state.holdingCostsMonthly}
        displayValue={`${fmt(state.holdingCostsMonthly)}/mo`}
        min={0}
        max={3000}
        onChange={(v) => up('holdingCostsMonthly', v)}
      />
      <SliderRow
        label="ARV"
        value={state.arv}
        displayValue={fmt(state.arv)}
      ...
      <SliderRow
        ...
        min={65}
        max={80}
        onChange={(v) => up('refinanceLtv', v / 100)}
        parseInput={(s) => parseFloat(s.replace(/[^0-9.]/g, ''))}
      />
      <SliderRow
        label="Refi Rate"
        ...
        max={10}
        step={0.25}
        onChange={(v) => up('refinanceInterestRate', v / 100)}
        parseInput={(s) => parseFloat(s.replace(/[^0-9.]/g, ''))}
      />
      ...
      <Row
        ...
        value={fmt(cashLeft)}
        color={cashLeft <= 0 ? '#10B981' : C.heading}
      />
      <Row
        label="Capital Recycled"
        value={`${capitalRecycled.toFixed(1)}%`}
        color={capitalRecycled >= 100 ? '#10B981' : C.heading}
      />
      <Row
        label="Monthly Cash Flow"
        value={fmt(monthlyCF)}
        color={monthlyCF >= 0 ? C.blue : '#F43F5E'}
      />
      <Row
        label="Annual Cash Flow"
        value={fmt(annualCF)}
  ...
  return (
    <>
      ...
      <SliderRow
        ...
        min={0}
        max={40}
        onChange={(v) => up('purchaseDiscountPct', v / 100)}
        parseInput={(s) => parseFloat(s.replace(/[^0-9.]/g, ''))}
      />
      <SliderRow
        label="Closing Costs"
      ...
      <SliderRow
        ...
        min={1}
        max={12}
        onChange={(v) => up('rehabTimeMonths', Math.round(v))}
        parseInput={(s) => parseInt(s.replace(/[^0-9]/g, ''), 10)}
      />
      <SliderRow
        label="ARV"
      ...
      <SliderRow
        ...
        min={15}
        max={180}
        onChange={(v) => up('daysOnMarket', Math.round(v))}
      />
      ...
      <SliderRow
        ...
        secondaryValue={`${(state.sellingCostsPct * 100).toFixed(0)}%`}
        displayValue={fmt(sellingCosts)}
        min={4}
        max={10}
        onChange={(v) => up('sellingCostsPct', v / 100)}
        parseInput={(s) => {
          const n = parseFloat(s.replace(/[^0-9.]/g, ''))
          const arv = grossProceeds || state.arv
          return arv > 0 ? (n / arv) * 100 : 0
        }}
      />
      ...
      <Row
        ...
        color={grossProfit >= 0 ? C.blue : '#F43F5E'}
      />
      <SliderRow
        label="Cap Gains Tax"
        value={state.capitalGainsRate * 100}
        secondaryValue={`${(state.capitalGainsRate * 100).toFixed(0)}%`}
        displayValue={fmt(capGains)}
        min={0}
        max={40}
        onChange={(v) => up('capitalGainsRate', v / 100)}
        parseInput={(s) => parseFloat(s.replace(/[^0-9.]/g, ''))}
      />
      ...
      <Row
        ...
        color={annualizedRoi >= 0 ? C.blue : '#F43F5E'}
      />
    </>
  )
}
...
function HouseHackWorksheet({
  ...
  const pi = typeof piRaw === 'number' && isFinite(piRaw) ? piRaw : combinedPiFallback
  ...
  return (
    <>
      ...
      <SliderRow
        ...
        min={2}
        max={8}
        onChange={(v) => up('totalUnits', Math.round(v))}
      />
      <SliderRow
        label="Owner Units"
        value={state.ownerOccupiedUnits}
        displayValue={`${state.ownerOccupiedUnits}`}
        min={1}
        max={2}
        onChange={(v) => up('ownerOccupiedUnits', Math.round(v))}
      />
      <ToggleRow
        label="Loan Type"
        options={[
          { id: 'fha', label: 'FHA' },
          { id: 'conventional', label: 'Conv' },
          { id: 'va', label: 'VA' },
        ]}
      ...
      <SliderRow
        ...
        displayValue={`${fmt(state.avgRentPerUnit)}/mo`}
        min={500}
        max={5000}
        onChange={(v) => up('avgRentPerUnit', v)}
      />
      <SliderRow
        label="Vacancy"
        value={state.vacancyRate * 100}
      ...
      <SliderRow
        ...
        displayValue={`${fmt(state.currentHousingPayment)}/mo`}
        min={0}
        max={5000}
        onChange={(v) => up('currentHousingPayment', v)}
      />
      <Row label="Rented Units" value={`${Math.round(rentedUnits)}`} />
      <Row label="Gross Rental" value={`${fmt(grossRental)}/mo`} />
      <NegRow
        label={`Vacancy (${Math.round(state.vacancyRate * 100)}%)`}
        value={`${fmt(grossRental - effectiveRental)}/mo`}
      />
      ...
      <TotalRow
        ...
        value={
          effectiveCost <= 0 ? `+${fmt(Math.abs(effectiveCost))}/mo` : `${fmt(effectiveCost)}/mo`
        }
      />
      <div className="mt-2" />
      <Row
        label="vs Current Housing"
        value={savings > 0 ? `+${fmt(savings)}/mo savings` : `${fmt(savings)}/mo`}
        color={savings >= 0 ? '#10B981' : '#F43F5E'}
      />
      <Row
        label="Housing Offset"
        value={`${offset.toFixed(0)}%`}
        color={offset >= 100 ? '#10B981' : C.heading}
      />
    </>
  )
}
  ...
  return (
    <>
      ...
      <SliderRow
        ...
        displayValue={fmt(state.estimatedRepairs)}
        min={0}
        max={200000}
        onChange={(v) => up('estimatedRepairs', v)}
      />
      <SliderRow
        field="contractPrice"
        label="Contract Price"
        value={state.contractPrice}
        displayValue={fmt(state.contractPrice)}
        min={25000}
        max={1500000}
        onChange={(v) => up('contractPrice', v)}
      />
      ...
      <SliderRow
        ...
        displayValue={fmt(state.earnestMoney)}
        min={100}
        max={10000}
        onChange={(v) => up('earnestMoney', v)}
      />
      <SliderRow
        label="Inspection"
        ...
        min={7}
        max={30}
        onChange={(v) => up('inspectionPeriodDays', Math.round(v))}
      />
      <SliderRow
        label="Days to Close"
        ...
        min={21}
        max={90}
        onChange={(v) => up('daysToClose', Math.round(v))}
      />
      ...
      <SliderRow
        ...
        displayValue={fmt(state.assignmentFee)}
        min={5000}
        max={50000}
        onChange={(v) => up('assignmentFee', v)}
      />
      <SliderRow
        label="Marketing"
        value={state.marketingCosts}
        displayValue={fmt(state.marketingCosts)}
        min={0}
        max={5000}
        onChange={(v) => up('marketingCosts', v)}
      />
      <SliderRow
        label="Closing Costs"
        value={state.closingCosts}
        displayValue={fmt(state.closingCosts)}
        min={0}
        max={2000}
        onChange={(v) => up('closingCosts', v)}
      />
      ...
      <Row
        label="Buyer Profit"
        value={fmt(endBuyerProfit)}
        color={endBuyerProfit >= 0 ? '#10B981' : '#F43F5E'}
      />
...
function BigCard({
  ...
  return (
    <div
      ...
      style={{ background: 'var(--surface-card)', border: `1.5px solid ${borderClr}` }}
    >
      <div className="text-xs font-bold uppercase tracking-wider" style={{ color: clr }}>
        {title}
      </div>
      <div className="text-[11px] mt-0.5" style={{ color: C.body }}>
        {subtitle}
      </div>
      <div
        className="text-[1.45rem] sm:text-[1.65rem] font-bold mt-3 tabular-nums leading-tight"
        style={{ color: clr }}
      >
        {fmtParen(annual)}
...
function ValueCard({
  ...
  return (
    ...
    >
      ...
      <div className="text-[1.65rem] font-bold mt-3 tabular-nums leading-tight" style={{ color }}>
        {value}
      </div>
    </div>
  )
  ...
  return (
    ...
    >
      <div className="flex justify-between items-baseline">
        <span className="text-xs font-bold uppercase tracking-wider" style={{ color: C.heading }}>
          {title}
        </span>
        <span className="text-xl font-bold tabular-nums" style={{ color: C.heading }}>
          {value}
        </span>
      </div>
...
function StatusBadgeCard({ title, value, pass }: { title: string; value: string; pass: boolean }) {
  ...
  return (
    ...
    >
      <div className="flex justify-between items-baseline">
        ...
        <span className="text-xl font-bold tabular-nums" style={{ color: clr }}>
          {value}
        </span>
      </div>
    ...
    return (
      <div className="grid grid-cols-2 gap-3 mt-5">
        ...
        <ValueCard
          ...
          value={`${roi.toFixed(1)}%`}
          color={roi >= 0 ? '#10B981' : '#F43F5E'}
        />
        <BadgeCard
          title="ANN. ROI"
          value={`${annualizedRoi.toFixed(1)}%`}
          target="30%"
          isGood={annualizedRoi >= 30}
        />
  ...
  if (strategyType === 'wholesale') {
    ...
    return (
      <div className="grid grid-cols-2 gap-3 mt-5">
        <ValueCard
          title="NET PROFIT"
          subtitle="Your Assignment Profit"
          value={fmt(netProfit)}
          color={netProfit >= 0 ? '#10B981' : '#F43F5E'}
        />
        <ValueCard
          title="ROI"
          subtitle="Return on Cash at Risk"
          value={`${roi.toFixed(1)}%`}
          color={roi >= 0 ? '#10B981' : '#F43F5E'}
        />
        <BadgeCard
          title="ANN. ROI"
          value={`${annROI.toFixed(0)}%`}
          target="500%"
          isGood={annROI >= 500}
        />
        <BadgeCard
          title="BUYER ROI"
          value={`${buyerROI.toFixed(1)}%`}
          target="20%"
          isGood={buyerROI >= 20}
        />
      </div>
...
export interface DealMakerWorksheetProps {
  ...
  /** Opens the Full Report print view (Strategy / Deal Maker worksheet footer). */
  onExportPDF?: () => void
  flushWithinParent?: boolean
  /**
   * Worksheet state-field names whose value the most recently applied Three
   * Paths structure changed vs the prior baseline. Strategy page passes this
   * in; the standalone DealMaker tab does not, so highlights are silently
   * disabled there.
   */
...
export function DealMakerWorksheet({
  ...
  const highlightContextValue = useMemo<ReadonlySet<string>>(
    () => highlightedFields ?? EMPTY_HIGHLIGHTS,
    [highlightedFields],
  )
  ...
  return (
    <WorksheetHighlightContext.Provider value={highlightContextValue}>
      <section className={`${flushWithinParent ? '' : 'mx-4 sm:mx-6'} pb-24 sm:pb-28`}>
        ...
        >
          ...
          {strategyType === 'ltr' && (
            <LTRWorksheet
              state={state as LTRDealMakerState}
          ...
          {strategyType === 'str' && (
            <STRWorksheet
              state={state as STRDealMakerState}
              metrics={metrics as STRMetrics}
              listPrice={listPrice}
              up={updateState}
            />
          )}
    ...
    </WorksheetHighlightContext.Provider>
  )
}
