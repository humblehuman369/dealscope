...
export function buildLTRState(
  ...
  return {
    buyPrice: price,
    downPaymentPercent: pick(f?.down_payment_pct, 0.2),
    ...
    managementRate: pick(o?.property_management_pct, 0.0),
...
export function buildSTRState(
  ...
  return {
    ...
    closingCostsPercent: pick(
      f?.closing_costs_pct,
      DEFAULT_STR_DEAL_MAKER_STATE.closingCostsPercent,
    ),
    ...
    arv: basePrice * 1.0,
    furnitureSetupCost: pick(
      s?.furniture_setup_cost,
      DEFAULT_STR_DEAL_MAKER_STATE.furnitureSetupCost,
    ),
    cleaningFeeRevenue: pick(
      s?.cleaning_fee_revenue,
      DEFAULT_STR_DEAL_MAKER_STATE.cleaningFeeRevenue,
    ),
    avgLengthOfStayDays: pick(
      s?.avg_length_of_stay_days,
      DEFAULT_STR_DEAL_MAKER_STATE.avgLengthOfStayDays,
    ),
...
export function buildBRRRRState(
  ...
  return {
    ...
    downPaymentPercent: pick(
      f?.down_payment_pct,
      DEFAULT_BRRRR_DEAL_MAKER_STATE.downPaymentPercent,
    ),
    closingCostsPercent: pick(
      f?.closing_costs_pct,
      DEFAULT_BRRRR_DEAL_MAKER_STATE.closingCostsPercent,
    ),
    ...
    holdingPeriodMonths: pick(
      r?.holding_period_months,
      DEFAULT_BRRRR_DEAL_MAKER_STATE.holdingPeriodMonths,
    ),
    arv: basePrice * 1.15,
    ...
    refinanceTermYears: pick(
      b?.refinance_term_years,
      DEFAULT_BRRRR_DEAL_MAKER_STATE.refinanceTermYears,
    ),
...
export function buildFlipState(
  ...
  const purchaseDiscountPct = pick(
    fl?.purchase_discount_pct,
    DEFAULT_FLIP_DEAL_MAKER_STATE.purchaseDiscountPct,
  )
  ...
  return {
    ...
    closingCostsPercent: pick(
      f?.closing_costs_pct,
      DEFAULT_FLIP_DEAL_MAKER_STATE.closingCostsPercent,
    ),
...
export function buildHouseHackState(
  ...
  return {
    ...
    ownerUnitMarketRent: property.rent ? Math.round(property.rent / 4) : 1500,
    avgRentPerUnit: property.rent ? Math.round(property.rent / 4) : 1500,
    downPaymentPercent: pick(
      hh?.fha_down_payment_pct,
    ...
    closingCostsPercent: pick(
      f?.closing_costs_pct,
      DEFAULT_HOUSEHACK_DEAL_MAKER_STATE.closingCostsPercent,
    ),
