...
/**
 * IQ Verdict Page
 * Route: /app/verdict?address=... OR /app/verdict?propertyId=...
 * 
...
function VerdictContent() {
  ...
  useEffect(() => {
    if (!isLoading && property && analysis && searchParams.get('openDealMaker') === '1') {
      ...
      router.replace(`/app/deal-maker?address=${encodeURIComponent(fullAddress)}&from=verdict`)
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleNavigateToDealMaker = useCallback(() => {
    ...
    const stateZip = [property.state, property.zip].filter(Boolean).join(' ')
    const fullAddress = [property.address, property.city, stateZip].filter(Boolean).join(', ')
    router.push(`/app/deal-maker?address=${encodeURIComponent(fullAddress)}&from=verdict`)
  }, [property, router])
