...
const L: P = {
  ...
  warn: '#D97706',
  border: '#E2E8F0',
  colors: ['#0FA4E9', '#10B981', '#F59E0B', '#F43F5E', '#8B5CF6', '#0FA4E9'],
}
const D: P = {
  bg: '#000000',
  card: '#0C1220',
...
function donut(segs: [string, number][], p: P, label: string, value: string, sz = 200): string {
  const tot = segs.reduce((s, [, v]) => s + v, 0)
  if (!tot) return ''
  const cx = sz / 2,
    cy = sz / 2,
    oR = sz / 2 - 8,
  ...
  for (let i = 0; i < segs.length; i++) {
    ...
    const r = Math.PI / 180
    const x1 = cx + mR * Math.cos((s - 90) * r),
      y1 = cy + mR * Math.sin((s - 90) * r),
      x2 = cx + mR * Math.cos((e - 90) * r),
      y2 = cy + mR * Math.sin((e - 90) * r)
    ps.push(
      `<path d="M ${x1.toFixed(1)} ${y1.toFixed(1)} A ${mR.toFixed(1)} ${mR.toFixed(1)} 0 ${sw2 > 180 ? 1 : 0} 1 ${x2.toFixed(1)} ${y2.toFixed(1)}" fill="none" stroke="${p.colors[i % p.colors.length]}" stroke-width="${sw.toFixed(1)}" stroke-linecap="round"/>`,
    )
    a += sw2
  }
...
function narrativeCover(d: Proforma): string {
  const pr = d.property,
    tp = ptypeLabel(pr.property_type),
    st = d.strategy_type.toUpperCase()
  ...
  if (pr.lot_size > 0) {
    const ac = pr.lot_size / 43560
    if (ac >= 0.1) t += ` on a ${fmt(pr.lot_size)}-square-foot lot (${ac.toFixed(2)} acres)`
  }
  return t + `. Analyzed under a ${st} investment strategy.`
}

function narrativeOverview(d: Proforma): string {
  const coc = d.metrics.cash_on_cash_return,
    pr = d.property,
    ac = d.acquisition,
    inc = d.income
  const assess =
    coc >= 8
      ? 'a strong cash-flowing investment opportunity'
      : coc >= 0
        ? 'a moderate investment opportunity with positive returns'
        : 'a wealth-building investment opportunity focused on long-term appreciation'
  ...
  if (d.metrics.price_per_sqft > 0)
    t += `carries a price per square foot of $${fmt(d.metrics.price_per_sqft)}. `
...
function narrativeExpense(d: Proforma): string {
  ...
  t +=
    ratio >= 35 && ratio <= 45
      ? ', a healthy expense ratio for residential properties. '
      : ratio < 35
        ? ', a favorable ratio indicating efficient operations. '
        : ', which is elevated relative to typical benchmarks. '
  t +=
    exp.insurance > exp.property_taxes
      ? `Insurance costs are the largest category at ${$(exp.insurance)}, `
      : `Property taxes are the largest category at ${$(exp.property_taxes)}, `
...
function narrativeMetrics(d: Proforma): string {
  const m = d.metrics,
    r = d.returns
  let t = ''
  const cap = m.cap_rate,
    coc = m.cash_on_cash_return
  if (cap >= 6 && coc >= 8)
    t +=
      'This investment demonstrates strong operational fundamentals with above-benchmark returns on both an unlevered and levered basis. '
  else if (cap >= 4 && coc >= 0)
    t +=
      'The investment shows moderate operational performance. While first-year metrics reflect current market conditions, the property is positioned for improving returns. '
  else
    t +=
      'First-year operational metrics reflect the challenges of current market pricing and financing conditions. '
  if (m.dscr >= 1.25)
    t += `A DSCR of ${m.dscr.toFixed(2)} indicates the property's income comfortably covers debt obligations with a healthy safety margin. `
  else if (m.dscr >= 1.0)
    t += `A DSCR of ${m.dscr.toFixed(2)} indicates the property's income covers debt obligations, though with limited margin. `
  else
    t += `A DSCR below 1.0 (${m.dscr.toFixed(2)}) indicates the property's operational income does not fully cover debt obligations in year one. `
  if (r.irr > 0) {
    t += `The ${pct(r.irr)} projected IRR demonstrates `
    t += r.irr >= 15 ? 'excellent' : r.irr >= 10 ? 'solid' : 'moderate'
    t +=
...
function narrativeProjections(d: Proforma): string {
  const proj = d.projections,
    yrs = proj.annual_projections.length
  let posYr: number | null = null
  for (let i = 0; i < proj.annual_projections.length; i++) {
    if (proj.annual_projections[i].pre_tax_cash_flow >= 0) {
      posYr = i + 1
      break
    }
  }
  const initEq = d.financing.down_payment,
    finalEq = proj.equity_positions[proj.equity_positions.length - 1] || initEq
  const eqGrowth = initEq > 0 ? ((finalEq - initEq) / initEq) * 100 : 0
  const initVal = d.acquisition.purchase_price,
    finalVal = proj.property_values[proj.property_values.length - 1] || initVal
  let t = ''
  if (posYr && posYr > 1)
    t += `The property transforms from negative to positive cash flow by year ${posYr} as rent growth at ${pct(proj.rent_growth_rate, 1)} annually outpaces expense growth at ${pct(proj.expense_growth_rate, 1)}. `
  else if (posYr === 1)
    t += `The property generates positive cash flow from year one, with returns strengthening as rent growth at ${pct(proj.rent_growth_rate, 1)} outpaces expenses. `
  else
    t += `Over the ${yrs}-year projection, rent growth at ${pct(proj.rent_growth_rate, 1)} works to offset expense growth at ${pct(proj.expense_growth_rate, 1)}. `
...
function narrativeExit(d: Proforma): string {
  ...
  if (e.accumulated_depreciation > 0)
    t += `Tax implications are significant, with accumulated depreciation of ${$(e.accumulated_depreciation)} subject to recapture at 25%, plus capital gains taxes on the remaining ${$(e.capital_gain)} profit. `
  ...
  t +=
    'Investors should consider 1031 exchange opportunities to defer capital gains taxation and reinvest proceeds into larger properties, accelerating portfolio growth.'
  return t
}

function narrativeSensitivity(d: Proforma): string {
  const s = d.sensitivity
  let t = 'Sensitivity analysis examines how changes in key variables affect investment returns. '
  if (s.purchase_price?.length) {
    const best = s.purchase_price.reduce((a, b) => (a.irr > b.irr ? a : b)),
      worst = s.purchase_price.reduce((a, b) => (a.irr < b.irr ? a : b))
    t += `Purchase price scenarios show IRR ranging from ${pct(worst.irr)} to ${pct(best.irr)}, demonstrating the impact of acquisition pricing on total returns. `
  }
  if (s.rent?.length) {
    const best = s.rent.reduce((a, b) => (a.irr > b.irr ? a : b)),
      worst = s.rent.reduce((a, b) => (a.irr < b.irr ? a : b))
    t += `Rent variation scenarios project IRR between ${pct(worst.irr)} and ${pct(best.irr)}, highlighting the sensitivity of returns to rental income assumptions. `
  }
  t +=
    'These scenarios help quantify risk and identify the variables with the greatest impact on investment performance.'
  return t
}
...
function kvTotal(label: string, value: string) {
  return `<div class="kv kv-total"><span>${label}</span><span>${value}</span></div>`
}
...
function metricCard(label: string, value: string, desc: string) {
  return `<div class="m-card"><div class="m-val">${value}</div><div class="m-lbl">${label}</div><div class="m-desc">${desc}</div></div>`
}
...
function buildReport(d: Proforma, theme: string, photos: string[]): string {
  ...
  const expSegs: [string, number][] = [
    ...
    [
      'Other',
      exp.utilities + exp.landscaping + exp.pest_control + (exp.hoa_fees || 0) + exp.other_expenses,
    ],
  ...
  const sensBlock = (
    ...
    return `<div class="sens-block"><h4>${title}</h4><table class="tbl"><thead><tr><th>Change</th><th>Value</th><th>IRR</th><th>CoC Return</th><th>Net Profit</th></tr></thead><tbody>${rows
      .map((s) => {
        const c = s.irr >= 0 ? p.pos : p.neg
        return `<tr><td>${s.change_percent > 0 ? '+' : ''}${pct(s.change_percent, 0)}</td><td>${$(s.absolute_value)}</td><td style="color:${c}">${pct(s.irr)}</td><td style="color:${c}">${pct(s.cash_on_cash)}</td><td style="color:${c}">${sign$(s.net_profit)}</td></tr>`
      })
  ...
  const photoHTML =
    ph.length > 0
      ? `<div class="photos photos-${Math.min(ph.length, 5)}">${ph.map((u, i) => `<div class="ph${i === 0 ? ' ph-main' : ''}"><img src="${u}" alt=""/></div>`).join('')}</div>`
      : ''
  ...
  const pgFt = (n: number) =>
    `<div class="pg-foot"><span>DealGapIQ Property Report</span><span>${dateStr}</span><span>Page ${n} of ${N}</span></div>`
...
/* --- Cover --- */
.brand-bar{height:4px;background:linear-gradient(90deg,${p.brand},${dk ? '#0FA4E9' : '#0284c7'});margin:-0.5in -0.55in calc(0.5in - 4px);width:calc(100% + 1.1in)}
...
.sec-divider{height:2px;background:linear-gradient(90deg,${p.brand},${dk ? '#0FA4E9' : '#0284c7'});border-radius:2px;margin:16px 0 18px}
...
export async function GET(request: NextRequest) {
  ...
  // Forward auth cookies so the backend can enforce Pro gating
  ...
  try {
    ...
    const [proformaRes, photosRes] = await Promise.all([
      fetch(`${BACKEND_URL}/api/v1/proforma/property/${propertyId}?${backendParams}`, {
        headers,
        cache: 'no-store',
      }),
      fetch(`${BACKEND_URL}/api/v1/photos?property_id=${propertyId}`, {
    ...
    if (!proformaRes.ok) {
      ...
      return new NextResponse(
        `<html><body style="font-family:sans-serif;padding:40px;text-align:center"><h2>Report generation failed</h2><p>Status: ${proformaRes.status}</p><p>${err}</p></body></html>`,
    ...
    try {
      if (photosRes?.ok) {
        ...
        if (pd.success && Array.isArray(pd.photos))
          photos = pd.photos.map((x: { url?: string }) => x.url).filter(Boolean)
      }
    } catch {}

    return new NextResponse(buildReport(proforma, theme === 'dark' ? 'dark' : 'light', photos), {
      status: 200,
      headers: { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' },
    })
  } catch (err) {
    return new NextResponse(
      `<html><body style="font-family:sans-serif;padding:40px;text-align:center"><h2>Error</h2><p>${err instanceof Error ? err.message : String(err)}</p></body></html>`,
