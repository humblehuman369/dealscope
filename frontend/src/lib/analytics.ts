...
import { REHAB_UNIT_COSTS, type RehabUnitCostId } from './rehabCostBook'
...
// Costs are sourced from the shared cost book (single source of truth).
// This file owns only the line-item metadata (name, icon, default quantity).
function item(
  id: RehabUnitCostId,
  name: string,
  defaultQuantity: number,
): RehabItem {
  const c = REHAB_UNIT_COSTS[id]
  return {
    id,
    name,
    lowCost: c.low,
...
export const REHAB_CATEGORIES: RehabCategory[] = [
  {
    ...
    icon: '🍳',
    items: [
      item('cabinets', 'Cabinets', 1),
      item('countertops', 'Countertops', 1),
      item('appliances', 'Appliances Package', 1),
      item('backsplash', 'Backsplash', 1),
      item('sink_faucet', 'Sink & Faucet', 1),
    ],
  },
  {
    id: 'bathroom',
    ...
    icon: '🚿',
    items: [
      item('full_bath', 'Full Bath Remodel', 1),
      item('half_bath', 'Half Bath Remodel', 1),
      item('vanity', 'Vanity & Sink', 1),
      item('toilet', 'Toilet', 1),
      item('tub_shower', 'Tub/Shower', 1),
    ],
  },
  {
    id: 'flooring',
    ...
    icon: '🏠',
    items: [
      item('lvp', 'Luxury Vinyl Plank', 1500),
      item('hardwood', 'Hardwood', 1500),
      item('tile', 'Tile', 200),
      item('carpet', 'Carpet', 500),
    ],
  },
  {
    id: 'paint',
    ...
    icon: '🎨',
    items: [
      item('interior_paint', 'Interior Paint', 2000),
      item('exterior_paint', 'Exterior Paint', 1),
      item('drywall_repair', 'Drywall Repair', 3),
    ],
  },
  {
    id: 'systems',
    ...
    icon: '⚡',
    items: [
      item('hvac', 'HVAC System', 1),
      item('water_heater', 'Water Heater', 1),
      item('electrical_panel', 'Electrical Panel', 1),
      item('plumbing_repipe', 'Plumbing Repipe', 1),
    ],
  },
  {
    id: 'exterior',
    ...
    icon: '🏡',
    items: [
      item('roof', 'Roof Replacement', 1),
      item('siding', 'Siding', 1),
      item('windows', 'Windows', 10),
      item('front_door', 'Front Door', 1),
      item('landscaping', 'Landscaping', 1),
    ],
  },
  {
    id: 'other',
    ...
    icon: '🔧',
    items: [
      item('permits', 'Permits', 1),
      item('dumpster', 'Dumpster Rental', 2),
      item('cleaning', 'Deep Cleaning', 1),
      item('staging', 'Staging', 1),
    ],
  },
]
