...
function calculateSTRMetrics(
  ...
  const cleaningRevenue = (inputs.cleaningFeeRevenue || 0) * (nightsOccupied / 3) // Assume 3-day avg stay
...
function calculateHouseHackMetrics(
  ...
  return {
    ...
    holdingCosts: null,
    profitMargin: null,
    effectiveHousingCost: effectiveHousingCost,
    savings: savings,
    reduction: reduction,
...
export function calculateMetric(
  ...
  return {
    id: metricId,
    value,
    formatted: formatMetricValue(value, definition.format),
    label: definition.label,
  }
}

/**
 * Calculate all 3 metrics for a strategy and price target
 */
