...
export function getDealOpportunityScoreFromTarget(
  ...
  return calculateDealOpportunityScore(iqTarget.incomeValue, listPrice, listingInfo)
}
...
function calculateLTRMetrics(purchasePrice: number, a: TargetAssumptions) {
  ...
  const totalOpEx =
    a.propertyTaxes +
    a.insurance +
    annualGrossRent * a.managementPct +
    annualGrossRent * a.maintenancePct
...
function calculateSTRMetrics(purchasePrice: number, a: TargetAssumptions) {
  ...
  const managementFee = annualGrossRent * 0.2
...
function calculateBRRRRMetrics(purchasePrice: number, a: TargetAssumptions) {
  const initialCash = purchasePrice * 0.3 + a.rehabCost + purchasePrice * a.closingCostsPct
  const allInCost = purchasePrice + a.rehabCost + purchasePrice * a.closingCostsPct
  const refinanceLoanAmount = a.arv * 0.75
  const cashBack = refinanceLoanAmount - purchasePrice * 0.7
...
function calculateFlipMetrics(purchasePrice: number, a: TargetAssumptions) {
  ...
  const holdingCosts =
    purchasePrice * (a.interestRate / 12) * a.holdingPeriodMonths +
  ...
  // 70% Rule check
  const maxPurchase70Rule = a.arv * 0.7 - a.rehabCost
...
function calculateHouseHackMetrics(purchasePrice: number, a: TargetAssumptions) {
  ...
  const monthlyExpenses =
    ...
    monthlyInsurance +
    pmi +
    monthlyRentalIncome * a.vacancyRate +
    monthlyRentalIncome * a.maintenancePct
...
function calculateWholesaleMetrics(purchasePrice: number, a: TargetAssumptions) {
  ...
  const mao = a.arv * 0.7 - a.rehabCost - wholesaleFee
...
export function calculateLTRTarget(a: TargetAssumptions): IQTargetResult {
  ...
  let searchLow = listPrice * 0.3
  let searchHigh = listPrice * 1.1
...
export function calculateBRRRRTarget(a: TargetAssumptions): IQTargetResult {
  ...
  let low = listPrice * 0.5
  let high = listPrice * 0.8
  ...
  if (
    lowMetrics.cashRecoveryPercent > targetRecovery &&
    highMetrics.cashRecoveryPercent < targetRecovery
  ) {
    for (let i = 0; i < 30; i++) {
      const mid = (searchLow + searchHigh) / 2
...
export function calculateFlipTarget(a: TargetAssumptions): IQTargetResult {
  ...
  const maxPer70Rule = a.arv * 0.7 - a.rehabCost
...
export function calculateHouseHackTarget(a: TargetAssumptions): IQTargetResult {
  ...
  if (
    lowMetrics.effectiveHousingCost < typicalRent &&
    highMetrics.effectiveHousingCost > typicalRent
  ) {
    for (let i = 0; i < 30; i++) {
      const mid = (searchLow + searchHigh) / 2
  ...
  return {
    ...
    incomeValue: Math.round(incomeValue / 1000) * 1000,
    incomeValuePercent: (incomeValue / listPrice) * 100,
    rationale: isFreeHousing
      ? 'At this price you live for'
      : 'At this price your housing cost is only',
    highlightedMetric: isFreeHousing
      ? 'FREE'
      : `${formatCurrency(finalMetrics.effectiveHousingCost)}/mo`,
...
export function calculateWholesaleTarget(a: TargetAssumptions): IQTargetResult {
  ...
  targetPrice = Math.max(targetPrice, listPrice * 0.5) // Don't go below 50% of list
...
export function calculateIQTarget(
