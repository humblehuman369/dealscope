    ...
    <div className={`min-h-screen overflow-x-hidden transition-colors duration-300 ${
      ...
      >
        ...
        <div className="relative z-10 text-center mb-8">
          ...
          <p className={`text-sm ${isDark ? 'text-[#8892a0]' : 'text-gray-500'}`}>
        ...
        {/* Social Proof */}
        <div className={`relative z-10 flex items-center justify-center gap-6 py-6 border-t ${
          isDark ? 'border-white/10' : 'border-gray-200'
        }`}>
          <div className="text-center">
            <div className="text-2xl font-bold text-accent-500">10K+</div>
            <div className={`text-[11px] ${isDark ? 'text-[#8892a0]' : 'text-gray-500'}`}>Properties Analyzed</div>
          </div>
          <div className={`w-px h-10 ${isDark ? 'bg-white/10' : 'bg-gray-200'}`} />
          <div className="text-center">
            <div className="text-2xl font-bold text-emerald-500">$2.4M</div>
            <div className={`text-[11px] ${isDark ? 'text-[#8892a0]' : 'text-gray-500'}`}>Profit Identified</div>
          </div>
          <div className={`w-px h-10 ${isDark ? 'bg-white/10' : 'bg-gray-200'}`} />
          <div className="text-center">
            <div className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>60s</div>
            <div className={`text-[11px] ${isDark ? 'text-[#8892a0]' : 'text-gray-500'}`}>Avg Analysis</div>
          </div>
        </div>
        ...
        <div className="relative z-10 mt-6">
          ...
          <p className={`text-center text-[12px] mt-3 ${isDark ? 'text-[#8892a0]' : 'text-gray-500'}`}>
      ...
      {/* Bottom Features Bar */}
      <div 
        className={`flex justify-around py-6 px-5 ${isDark ? 'bg-[#061324]' : 'bg-gray-100'}`}
      >
...
function StrategyCard({ name, roi, icon: Icon, featured, multiplier, profit, isDark }: StrategyCardProps) {
  return (
    <div 
      className={`relative rounded-2xl p-4 overflow-hidden transition-colors
        ${featured 
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        >
          <Icon className={`w-5 h-5 ${featured ? 'text-accent-400' : 'text-brand-500'}`} />
        </div>
      ...
      <div className="flex items-end justify-between">
        {/* ROI */}
        <div>
          <div className={`text-[11px] mb-0.5 uppercase tracking-wide ${
      ...
      {profit && (
        <div className={`mt-3 pt-3 border-t flex items-center justify-between ${
          ...
          <span className={`text-[11px] ${isDark ? 'text-[#8892a0]' : 'text-gray-500'}`}>Est. Profit</span>
          <span className={`text-sm font-bold ${featured ? 'text-accent-400' : 'text-emerald-500'}`}>
            {profit}
            {featured && multiplier && (
...
function FeatureItem({ icon: Icon, text, isDark }: FeatureItemProps) {
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <span className={`text-[12px] font-medium ${isDark ? 'text-white/80' : 'text-gray-600'}`}>{text}</span>
    </div>
  );
}
