...
import type {
  ...
  RentComp,
  CompsIdentifier,
  SubjectProperty as CompsSubjectProperty,
} from '@/lib/api/types'
import {
...
const getFreshnessBadge = (dateString: string, isSale: boolean) => {
  ...
  if (daysAgo <= 30)
    return { label: 'Recent', color: 'var(--status-positive)', bgColor: 'rgba(52,211,153,0.12)' }
  if (daysAgo > 90)
    return {
      label: isSale ? 'Older sale' : 'Older listing',
...
const cardBorderGlowHover =
  'hover:border-[var(--border-focus)] hover:shadow-[var(--shadow-card-hover)]'
...
function CompCard({
  ...
  return (
    <div
      className={`relative rounded-xl transition-all overflow-hidden bg-[var(--surface-base)] ${
        isSelected
    ...
    >
      ...
      <button
        ...
        className={`absolute top-2 left-2 z-10 min-w-[44px] min-h-[44px] rounded-full flex items-center justify-center transition-all ${
          isSelected
            ? 'bg-[var(--color-sky-dim)] border-2 border-[var(--accent-sky-light)]'
            : 'bg-[var(--surface-base)] border-2 border-[var(--text-heading)] hover:border-[var(--accent-sky-light)]'
        }`}
        aria-label={isSelected ? 'Deselect comp' : 'Select comp'}
      >
        <span
          className={`rounded-full flex items-center justify-center w-5 h-5 ${isSelected ? 'bg-[var(--accent-sky-light)]' : ''}`}
        >
          ...
          <div className="relative h-[80px] bg-[var(--surface-elevated)] rounded-tl-xl overflow-hidden">
            {comp.imageUrl && !imgFailed ? (
              ...
              <div className="w-full h-full flex items-center justify-center bg-[var(--surface-elevated)]">
                {isSale ? (
                  <Building2 className="w-5 h-5 text-[var(--text-heading)]" />
                ) : (
                  <Home className="w-5 h-5 text-[var(--text-heading)]" />
                )}
              </div>
            )}
        ...
        <div className="flex-1 p-3 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            <div className="min-w-0 pl-4">
              ...
              <p className="text-xs text-[var(--text-heading)] truncate">
                {comp.city}, {comp.state}
              </p>
            </div>
            {(() => {
              const compPrice = isSale
                ? (comp as SaleComp).salePrice
                : (comp as RentComp).monthlyRent
              const compPpsf = Number(
                isSale ? (comp as SaleComp).pricePerSqft : (comp as RentComp).rentPerSqft,
              )
              const priceMissing = !(compPrice > 0)
              return (
                <div className="text-right flex-shrink-0">
                  <p
                    className="text-sm font-bold text-[var(--text-heading)] tabular-nums"
                  ...
                  >
                    {priceMissing ? (
                      ...
                      <>
                        ...
                        {!isSale && (
                          <span className="text-xs font-normal text-[var(--text-heading)]">
                            /mo
                          </span>
                        )}
                      </>
          ...
          <div className="flex items-center justify-between gap-2 pl-4">
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[12px] text-[var(--text-heading)]">
                {isSale ? 'Sold' : 'Listed'} {formatDate(compDate)}
              </span>
              <span className="text-[11px] px-1.5 py-0.5 rounded bg-[var(--surface-elevated)] text-[var(--text-heading)] tabular-nums">
                {getDaysAgo(compDate)}
              </span>
              {freshness && (
                <span
            ...
            <button
              ...
              className="flex-shrink-0 text-[13px] text-[var(--accent-sky-light)] hover:opacity-80 font-medium flex items-center gap-0.5 pr-1"
            >
              Details{' '}
              <ChevronDown
      ...
      {isExpanded && (
        <div className="border-t border-[var(--border-subtle)] p-3 bg-[var(--surface-elevated)]/50">
          ...
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xs font-semibold text-[var(--text-heading)] uppercase">
              Match Score:
            </span>
            <span
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <h5 className="text-[10px] font-semibold text-[var(--accent-sky-light)] uppercase tracking-wide mb-2">
                Similarity
              </h5>
              ...
              <div className="space-y-1">
                {isSale &&
                  ...
                  [
                    ...
                    <div key={adj.label} className="flex justify-between text-xs">
                      ...
                      <span
                        className={`font-medium tabular-nums ${adj.value >= 0 ? 'text-[var(--status-positive)]' : 'text-[var(--status-negative)]'}`}
                      >
                        {adj.value >= 0 ? '+' : ''}
                        {formatCurrency(Math.round(adj.value))}
                      </span>
                    </div>
                  ))}
                {!isSale &&
                  ...
                  [
                    ...
                    <div key={adj.label} className="flex justify-between text-xs">
                      ...
                      >
                        {adj.value >= 0 ? '+' : ''}
                        {formatCurrency(Math.round(adj.value))}/mo
                      </span>
                    </div>
                  ))}
                <div className="flex justify-between text-xs pt-1 border-t border-[var(--border-subtle)]">
                  ...
                  <span className="font-bold text-[var(--accent-sky-light)] tabular-nums">
                    {formatCurrency(
                      (isSale ? (comp as SaleComp).salePrice : (comp as RentComp).monthlyRent) +
                        Math.round(isSale ? saleAdj?.total || 0 : rentAdj?.total || 0),
                    )}
                    {!isSale && '/mo'}
                  </span>
                </div>
  ...
  return (
    <div className={`bg-[var(--surface-base)] rounded-xl overflow-hidden ${largeCardBorderGlow}`}>
      ...
      >
        ...
        {isExpanded ? (
          <ChevronUp className="w-4 h-4 text-[var(--text-heading)]" />
        ) : (
          <ChevronDown className="w-4 h-4 text-[var(--text-heading)]" />
        )}
      </button>
      {isExpanded && (
        <div className="border-t border-[var(--border-subtle)] overflow-x-auto">
          <table className="w-full text-xs">
            <thead className="bg-[var(--surface-elevated)]/50">
              <tr>
                <th className="px-3 py-2 text-left font-semibold text-[var(--text-heading)]">
                  Address
                </th>
                <th className="px-3 py-2 text-right font-semibold text-[var(--text-heading)]">
                  {isSale ? 'Base' : 'Rent'}
                </th>
                <th className="px-3 py-2 text-right font-semibold text-[var(--text-heading)]">
                  Size
                ...
                <th className="px-3 py-2 text-right font-semibold text-[var(--accent-sky-light)]">
                  Adjusted
                </th>
                <th className="px-3 py-2 text-right font-semibold text-[var(--text-heading)]">
                  Weight
            ...
            <tbody>
              {compAdjustments.map((ca, idx) => (
                <tr
                  key={ca.compId}
                  className={idx % 2 === 0 ? 'bg-transparent' : 'bg-[var(--surface-elevated)]/30'}
                >
                  <td
                    className="px-3 py-2 text-[var(--text-body)] truncate max-w-[140px]"
                    title={ca.compAddress}
                  >
                    {ca.compAddress}
                  </td>
                  <td className="px-3 py-2 text-right tabular-nums text-[var(--text-body)]">
                    {isSale ? formatCompactCurrency(ca.basePrice) : `$${ca.basePrice}`}
                  </td>
                  <td
                    className={`px-3 py-2 text-right tabular-nums ${ca.sizeAdjustment >= 0 ? 'text-[var(--status-positive)]' : 'text-[var(--status-negative)]'}`}
                  >
                    {ca.sizeAdjustment >= 0 ? '+' : ''}
                    {isSale ? formatCompactCurrency(ca.sizeAdjustment) : `$${ca.sizeAdjustment}`}
                  </td>
                  <td
                    className={`px-3 py-2 text-right tabular-nums ${ca.bedroomAdjustment >= 0 ? 'text-[var(--status-positive)]' : 'text-[var(--status-negative)]'}`}
                  >
                    {ca.bedroomAdjustment >= 0 ? '+' : ''}
                    {isSale
                      ? formatCompactCurrency(ca.bedroomAdjustment)
                      : `$${ca.bedroomAdjustment}`}
                  </td>
                  <td
                    className={`px-3 py-2 text-right tabular-nums ${ca.bathroomAdjustment >= 0 ? 'text-[var(--status-positive)]' : 'text-[var(--status-negative)]'}`}
                  >
                    {ca.bathroomAdjustment >= 0 ? '+' : ''}
                    {isSale
                      ? formatCompactCurrency(ca.bathroomAdjustment)
                      : `$${ca.bathroomAdjustment}`}
                  </td>
                  {isSale && (
                    <td
                      className={`px-3 py-2 text-right tabular-nums ${ca.ageAdjustment >= 0 ? 'text-[var(--status-positive)]' : 'text-[var(--status-negative)]'}`}
                    >
                      {ca.ageAdjustment >= 0 ? '+' : ''}
                  ...
                  {isSale && (
                    <td
                      className={`px-3 py-2 text-right tabular-nums ${ca.lotAdjustment >= 0 ? 'text-[var(--status-positive)]' : 'text-[var(--status-negative)]'}`}
                    >
                      {ca.lotAdjustment >= 0 ? '+' : ''}
                  ...
                  <td className="px-3 py-2 text-right tabular-nums text-[var(--text-heading)]">
...
// MAIN COMPONENT
// ============================================
export function PriceCheckerIQScreen({
  property,
  initialView = 'sale',
}: PriceCheckerIQScreenProps) {
  ...
  const subject: SubjectProperty = useMemo(() => {
    ...
    const inferSqft =
      allComps.length > 0
        ? Math.round(allComps.reduce((s, c) => s + c.sqft, 0) / allComps.length)
        : 1500
    const inferYear =
      allComps.length > 0
        ? Math.round(
            allComps.reduce((s, c) => s + c.yearBuilt, 0) /
              allComps.filter((c) => c.yearBuilt > 0).length,
          ) || 2000
        : 2000
    const inferBeds =
      allComps.length > 0
        ? Math.round(allComps.reduce((s, c) => s + c.beds, 0) / allComps.length)
        : 3
    const inferBaths =
      allComps.length > 0
        ? Math.round(allComps.reduce((s, c) => s + c.baths, 0) / allComps.length)
        : 2
  ...
  const buildIdentifier = useCallback(
    (offset = 0, excludeZpids: string[] = []): CompsIdentifier => {
  ...
  const fetchSaleComps = useCallback(
  ...
  const fetchRentComps = useCallback(
    async (offset = 0, excludeZpids: string[] = []) => {
      setRentLoading(true)
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const saleIds =
        sold.length > 0 ? new Set(sold.slice(0, 3).map((c) => c.id)) : new Set<string | number>()
      const rentIds =
        rented.length > 0
          ? new Set(rented.slice(0, 3).map((c) => c.id))
  ...
  const handleApplyConsensusMode = useCallback(
    (mode: UnderwritingMode, value: number) => {
  ...
  const hasChangedFromOriginal = useMemo(() => {
    ...
    originalSaleComps,
    saleSelected,
    originalSaleSelected,
    rentComps,
    originalRentComps,
    rentSelected,
    originalRentSelected,
  ])
  ...
  const handleRefreshUnselected = async () => {
    ...
    const selectedZpids = currentComps
      .filter((c) => currentSelected.has(c.id))
  ...
  const handleRefreshComp = async (compId: string | number) => {
    ...
    const currentComps = isSale ? saleComps : rentComps
    const excludeZpids = currentComps
    ...
    <div className="min-h-screen bg-[var(--surface-base)] font-['Inter',sans-serif]">
      ...
      >
        ...
        <div className="mx-1 mt-4">
          ...
          <div
            className={`relative rounded-t-xl p-4 overflow-hidden bg-[var(--surface-base)] ${largeCardBorderGlow}`}
            style={{
              background:
                'radial-gradient(ellipse at 30% 0%, var(--color-teal-dim) 0%, transparent 60%), radial-gradient(ellipse at 80% 100%, var(--color-teal-dim) 0%, transparent 50%), var(--surface-base)',
            }}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2">
              <div className="col-span-2 sm:col-span-1 flex min-w-0 items-center gap-2">
                ...
                <div className="min-w-0">
                  <h3 className="text-[16px] sm:text-[18px] font-bold text-[var(--text-heading)]">
                    {isSale ? 'Appraisal Values' : 'Rental Appraisal'}
                  </h3>
                </div>
              </div>
              <div
              ...
              >
                <button
                  ...
                  onClick={() => {
                    setActiveView('sale')
                    setShowAdjGrid(false)
                    setExpandedComp(null)
                    setActiveUnderwriteMode(null)
                  }}
              ...
              <div className="text-center px-2 py-1 rounded-lg bg-[var(--surface-base)] border border-[var(--border-subtle)]">
                <div
                  ...
                  style={{
                    color:
                      (isSale ? saleAppraisal.confidence : rentAppraisal.confidence) >= 85
                        ? 'var(--accent-sky-light)'
                        : (isSale ? saleAppraisal.confidence : rentAppraisal.confidence) >= 70
                          ? 'var(--status-warning)'
                          : 'var(--status-negative)',
                  }}
                >
                  {loading
                    ? '...'
                    : `${isSale ? saleAppraisal.confidence : rentAppraisal.confidence}%`}
                </div>
          ...
          <div className="sticky top-[152px] z-40 overflow-hidden rounded-b-xl">
            <div
              className={`relative rounded-b-xl px-3 py-2 overflow-hidden bg-[var(--surface-base)] ${largeCardBorderGlow}`}
              style={{
                background:
                  'radial-gradient(ellipse at 30% 0%, var(--color-teal-dim) 0%, transparent 60%), radial-gradient(ellipse at 80% 100%, var(--color-teal-dim) 0%, transparent 50%), var(--surface-base)',
              }}
                ...
                <div className="bg-[var(--surface-base)] rounded-lg px-2.5 py-2 border border-[var(--border-subtle)] flex flex-col">
                  <div className="flex items-center justify-between mb-0.5">
                    ...
                    <button
                      onClick={() => {
                        if (isSale)
                          setSaleOverrideMarket(
                            saleOverrideMarket !== null ? null : saleAppraisal.marketValue,
                          )
                        else
                          setRentOverrideMarket(
                            rentOverrideMarket !== null ? null : rentAppraisal.marketRent,
                          )
                      }}
                  ...
                  {(isSale ? saleOverrideMarket : rentOverrideMarket) !== null ? (
                    <input
                      type="text"
                      value={formatCurrency(isSale ? saleOverrideMarket! : rentOverrideMarket!)}
                      onChange={(e) => {
                  ...
                  <div className="text-[13px] font-medium text-[var(--text-heading)] mt-0.5">
                    As-Is Condition
                  </div>
                    ...
                    {loading ? (
                      ...
                      <>
                        ...
                        <span className="text-[var(--status-positive)]">
                          (From {isSale ? saleAppraisal.compCount : rentAppraisal.compCount}{' '}
                          selected)
                        </span>
                      </>
                    )}
                ...
                <div className="bg-[var(--surface-base)] rounded-lg px-2.5 py-2 border border-[var(--border-subtle)] flex flex-col">
                  <div className="flex items-center justify-between mb-0.5">
                    ...
                    <button
                      onClick={() => {
                        if (isSale)
                          setSaleOverrideArv(saleOverrideArv !== null ? null : saleAppraisal.arv)
                        else
                          setRentOverrideImproved(
                            rentOverrideImproved !== null ? null : rentAppraisal.improvedRent,
                          )
                      }}
                  ...
                  {(isSale ? saleOverrideArv : rentOverrideImproved) !== null ? (
                    <input
                      type="text"
                      value={formatCurrency(isSale ? saleOverrideArv! : rentOverrideImproved!)}
                      onChange={(e) => {
                        const v = parseInt(e.target.value.replace(/[^0-9]/g, ''))
                        if (isNaN(v)) return
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-[13px] font-medium text-[var(--status-positive)]">
                      {isSale ? '+15% rehab premium' : '+10% condition premium'}
                    </span>
                  </div>
                  <div className="mt-auto flex justify-end pt-1">
                    <button
              ...
              <div className="flex items-center justify-between text-[10px] text-[var(--text-heading)] mb-1 px-0.5 gap-3">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className="tabular-nums font-medium text-[var(--text-heading)]">
                    ${isSale ? saleAppraisal.weightedAveragePpsf : rentAppraisal.rentPerSqft}/sqft
                    avg
                  </span>
                </div>
              </div>
        ...
        {hasValidSubject && !loading && !loadFailed && comps.length > 0 && (
          <div className="mx-1 mt-3 md:hidden">
            <div
              className={`bg-[var(--surface-base)] rounded-xl overflow-hidden ${largeCardBorderGlow}`}
            >
                  ...
                  <div className="flex items-center gap-3 text-[10px] text-[var(--text-heading)]">
                    ...
                    <span className="flex items-center gap-1">
                      <span
                        ...
                        style={{
                          backgroundColor: isSale ? 'var(--accent-sky)' : 'var(--accent-sky-light)',
                        }}
                      />
        ...
        {!hasValidSubject && (
          <div
            className={`mx-1 mt-3 rounded-xl p-6 text-center bg-[var(--surface-base)] ${cardBorderGlow}`}
          >
        ...
        {hasValidSubject && !loading && !loadFailed && comps.length === 0 && (
          <div
            className={`mx-1 mt-3 bg-[var(--surface-base)] rounded-xl p-6 text-center ${cardBorderGlow}`}
          >
            {isSale ? (
              <Building2 className="mx-auto mb-2 text-[var(--text-heading)] w-8 h-8" />
            ) : (
              <Home className="mx-auto mb-2 text-[var(--text-heading)] w-8 h-8" />
            )}
            <h3 className="text-sm font-semibold text-[var(--text-body)] mb-1">
              No {isSale ? 'Sale' : 'Rental'} Comps Found
            </h3>
        ...
        {mapLayout === 'top' && hasValidSubject && !loading && !loadFailed && comps.length > 0 && (
          <div className="hidden md:block mx-1 mt-3">
            <div
              className={`rounded-xl overflow-hidden bg-[var(--surface-base)] ${largeCardBorderGlow}`}
            >
        ...
        {hasValidSubject && !loading && !loadFailed && comps.length > 0 && (
          <div className="md:mt-3 md:mx-1">
            {/* Full-width: Adjustment Grid */}
            <div className="mx-1 md:mx-0">
              <AdjustmentGrid
                compAdjustments={
                  isSale ? saleAppraisal.compAdjustments : rentAppraisal.compAdjustments
                }
              ...
              >
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm text-[var(--text-heading)] flex-shrink-0">
                      {selectedIds.size} of {filteredComps.length} selected
                    </span>
                    ...
                    <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide min-w-0">
                      ...
                      <button
                        ...
                        disabled={
                          loading || selectedIds.size === 0 || selectedIds.size === comps.length
                        }
                      ...
                      <div className="flex rounded-lg bg-[var(--surface-elevated)]/50 border border-[var(--border-subtle)] p-0.5 flex-shrink-0">
                        {(
                          ...
                          <button
                            key={val}
                            onClick={() => setRecencyFilter(val)}
                            className={`px-3 py-1 text-xs font-medium rounded-md transition-colors whitespace-nowrap min-h-[44px] sm:min-h-0 ${
                              recencyFilter === val
                                ? 'bg-[var(--surface-base)] text-[var(--accent-sky-light)] border border-[var(--border-subtle)]'
                                : 'text-[var(--text-heading)] hover:text-[var(--text-heading)]'
                            }`}
                          >
                            {label}
                          </button>
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() =>
                            (isSale ? setSaleSelected : setRentSelected)(
                              new Set(filteredComps.map((c) => c.id)),
                            )
                          }
                          className="flex-shrink-0 min-h-[44px] px-3 py-2 sm:py-1 text-xs font-medium text-[var(--text-body)] hover:bg-[var(--surface-elevated)] rounded-lg whitespace-nowrap"
                        >
                          Select All
                        </button>
                        <button
                          onClick={() => (isSale ? setSaleSelected : setRentSelected)(new Set())}
                          className="flex-shrink-0 min-h-[44px] px-3 py-2 sm:py-1 text-xs font-medium text-[var(--text-body)] hover:bg-[var(--surface-elevated)] rounded-lg whitespace-nowrap"
                        >
                          Clear
                        </button>
                ...
                <div
                  className={`w-full min-w-0 px-1 sm:px-1 mt-3 space-y-3${
                    mapLayout === 'side' ? ' md:px-3 md:py-3 md:mt-0' : ''
                  }`}
                >
                  ...
                  <div className={`mt-2 p-3 rounded-lg bg-[var(--surface-base)] ${cardBorderGlow}`}>
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-[var(--text-heading)]">
                        {comps.filter((c) => c.distanceMiles <= 0.5).length} of {comps.length}{' '}
                        within 0.5 mi
                      </span>
                      <span
