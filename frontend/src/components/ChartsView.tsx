      ...
      <div className="bg-gradient-to-r from-[#0465f2] via-[#0354d1] to-[#0465f2] rounded-xl p-3 sm:p-4 text-white">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="min-w-0">
            <div className="text-white/80 text-[10px] uppercase tracking-wide">
              Annual Cash Flow
            </div>
            <div className="text-lg sm:text-xl font-bold">
              {formatCompactCurrency(yearData.cashFlow)}
            </div>
          </div>
          <div className="hidden sm:block text-2xl font-thin text-white/30">→</div>
          <div className="text-right min-w-0">
            <div className="text-white/80 text-[10px] uppercase tracking-wide">
              Year {selectedYear + 1} Cumulative
            </div>
      ...
      <div className="bg-[var(--surface-card)] rounded-xl border border-[var(--border-subtle)] p-3 sm:p-5">
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
          ...
          <div className="w-full sm:flex-1">
            <div className="text-sm font-medium text-[var(--text-heading)] mb-2 sm:mb-3">
                ...
                return (
                  <button
                    ...
                    className={`flex-1 rounded-t-lg transition-all duration-300 cursor-pointer hover:opacity-80 ${
                      isSelected
                        ? isPositive
                          ? 'bg-gradient-to-t from-[#0354d1] to-[#0465f2]'
                          : 'bg-gradient-to-t from-[#b42318] to-[#dc2626]'
                        : isPositive
                          ? 'bg-gradient-to-t from-[rgba(4, 101, 242,0.35)] to-[rgba(4, 101, 242,0.15)]'
                          : 'bg-gradient-to-t from-[rgba(180,35,24,0.35)] to-[rgba(180,35,24,0.12)]'
                    }`}
              ...
              {data.map((d, i) => (
                <div
                  ...
                  className={`flex-1 text-center text-[9px] sm:text-[10px] transition-colors ${
                    i === selectedYear
                      ? 'text-[var(--status-positive)] font-bold'
                      : 'text-[var(--text-muted)]'
                  }`}
                >
                  Y{d.year}
            ...
            {data.map((_, i) => (
              <button
                ...
                className={`flex-1 h-2 rounded-full transition-all duration-300 ${
                  i <= selectedYear ? 'bg-gradient-to-r from-[#0465f2] to-[#0465f2]' : 'bg-white/20'
...
function EquityMirrorChart({
  ...
  const equityComponents = [
    ...
    {
      label: 'Appreciation',
      value: yearData.equityFromAppreciation,
      color: '#0465f2',
      icon: TrendingUp,
    },
  ]
      ...
      <div className="bg-[var(--surface-card)] rounded-xl border border-[var(--border-subtle)] p-3 sm:p-5">
        <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
          ...
          <div className="w-full sm:flex-1 space-y-2.5">
            <div className="text-sm font-medium text-[var(--text-heading)] mb-3">
              Equity Breakdown
            </div>
            {(() => {
              cumulativePercent = 0
                ...
                <div key={i} className="flex items-center gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <span className="text-sm text-[var(--text-heading)] truncate">
                        {seg.label}
                      </span>
                      <span
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {crossoverYear > 0 && selectedYear >= crossoverYear && (
            <div className="flex items-center gap-1.5 px-2 py-1 bg-[rgba(4, 101, 242,0.12)] rounded-full">
              ...
              <span className="text-xs font-medium text-[var(--status-positive)]">
                Equity exceeds loan!
              </span>
            </div>
          )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 h-8 bg-[var(--surface-card)] rounded-lg overflow-hidden border border-[var(--border-default)]">
              <div
                ...
                style={{
                  width: `${(totalEquity / Math.max(totalEquity, yearData.loanBalance)) * 100}%`,
                }}
              >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 h-8 bg-[var(--surface-card)] rounded-lg overflow-hidden border border-[var(--border-default)]">
              <div
                ...
                style={{
                  width: `${(yearData.loanBalance / Math.max(totalEquity, yearData.loanBalance)) * 100}%`,
                }}
              >
                <span className="text-xs font-bold text-white">
                  {formatCompactCurrency(yearData.loanBalance)}
                </span>
              </div>
            </div>
          </div>
...
function TotalWealthDonutChart({
  data,
  totalCashInvested,
}: {
  data: YearlyProjection[]
  ...
  const segments = [
    {
      label: 'Cash Flow',
      value: Math.max(0, yearData.cumulativeCashFlow),
      color: '#0465f2',
      icon: Wallet,
    },
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-[#0465f2] via-[#07172e] to-[#0465f2] rounded-xl p-3 sm:p-4 text-white">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="text-right min-w-0">
            <div className="text-white/80 text-[10px] uppercase tracking-wide">
              Year {selectedYear + 1} Wealth
            </div>
            <div className="text-xl sm:text-2xl font-bold">
              {formatCompactCurrency(yearData.totalWealth)}
            </div>
          </div>
          <div className="text-right pl-3 border-l border-white/20 min-w-0">
          ...
          <div className="w-full sm:flex-1 space-y-2">
            {segments.map((seg, i) => {
              ...
              return (
                <div
                  key={i}
                  className="bg-[var(--surface-elevated)] dark:bg-navy-700/50 rounded-lg p-2.5"
                >
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      />
                      <span className="text-[13px] font-medium text-[var(--text-heading)] dark:text-white truncate">
                        {seg.label}
                      </span>
                      <span className="text-[13px] text-[var(--text-secondary)] dark:text-white">
                        {percent.toFixed(0)}%
                      </span>
                    </div>
                    <div
...
export default function ChartsView({ projections, totalCashInvested }: ChartsViewProps) {
  ...
  const tabs = [
    {
      ...
      label: 'Cash Flow',
      icon: DollarSign,
      value: year10.cumulativeCashFlow,
      color: 'emerald',
    },
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      {/* Header */}
      <div>
        <h2 className="text-base sm:text-lg font-semibold text-[var(--text-heading)] dark:text-white">
          Wealth Visualizer
        </h2>
        <p className="text-[13px] sm:text-[14px] text-[var(--text-secondary)] dark:text-white">
          Interactive 10-year investment analysis
        </p>
      </div>
        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`text-[11px] sm:text-[13px] px-1 sm:px-1.5 py-0.5 rounded whitespace-nowrap ${
                  isActive
                    ? 'bg-[var(--surface-elevated)] dark:bg-navy-500 text-[var(--text-heading)] dark:text-white'
                    : 'text-[var(--text-secondary)] dark:text-white'
                }`}
              >
      ...
      <div className="min-h-[350px] sm:min-h-[400px]">
        ...
        {activeChart === 'equity' && (
          <EquityMirrorChart data={projections} totalCashInvested={totalCashInvested} />
        )}
        {activeChart === 'wealth' && (
          <TotalWealthDonutChart data={projections} totalCashInvested={totalCashInvested} />
        )}
      </div>
    </div>
  )
