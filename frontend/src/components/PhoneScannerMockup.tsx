    ...
    <div className="scanner-mockup w-full max-w-[360px] lg:max-w-[460px] mx-auto">
      ...
      <div
        ...
        style={{
          ...
          border: isDark ? '1px solid rgba(77,208,225,0.2)' : '1px solid rgba(0,126,167,0.15)',
        }}
      >
        {/* Scanner Viewport */}
        <div className="scanner-viewport h-[160px] lg:h-[180px] flex items-center justify-center relative mb-4">
          <div className="scanner-frame w-[160px] lg:w-[180px] h-[140px] lg:h-[160px] relative">
            ...
            <div
              ...
              style={{
                background: isDark ? 'rgba(77,208,225,0.15)' : 'rgba(0,126,167,0.12)',
                filter: 'blur(20px)',
              }}
            />
            ...
            {/* Corner Brackets */}
            <div
              className={`corner corner-tl absolute top-0 left-0 w-7 h-7 border-t-[3px] border-l-[3px] rounded-tl-md animate-corner-pulse ${isDark ? 'border-accent-500' : 'border-accent-light'}`}
              style={{ animationDelay: '0s' }}
            />
            <div
              className={`corner corner-tr absolute top-0 right-0 w-7 h-7 border-t-[3px] border-r-[3px] rounded-tr-md animate-corner-pulse ${isDark ? 'border-accent-500' : 'border-accent-light'}`}
              style={{ animationDelay: '0.1s' }}
            />
            <div
              className={`corner corner-bl absolute bottom-0 left-0 w-7 h-7 border-b-[3px] border-l-[3px] rounded-bl-md animate-corner-pulse ${isDark ? 'border-accent-500' : 'border-accent-light'}`}
              style={{ animationDelay: '0.2s' }}
            />
            <div
              className={`corner corner-br absolute bottom-0 right-0 w-7 h-7 border-b-[3px] border-r-[3px] rounded-br-md animate-corner-pulse ${isDark ? 'border-accent-500' : 'border-accent-light'}`}
            ...
            <div
              ...
              style={{
                background: isDark
                  ? 'linear-gradient(90deg, transparent, var(--accent-sky), transparent)'
                  : 'linear-gradient(90deg, transparent, var(--accent-sky), transparent)',
                boxShadow: isDark
                  ? '0 0 15px var(--accent-sky), 0 0 30px var(--accent-sky)'
                  : '0 0 15px var(--accent-sky), 0 0 30px var(--accent-sky)',
              }}
            />
        ...
        >
          <div
            className={`text-[11px] font-bold tracking-[0.12em] uppercase mb-1 ${isDark ? 'text-accent-500' : 'text-accent-light'}`}
          >
            Property Located
          </div>
