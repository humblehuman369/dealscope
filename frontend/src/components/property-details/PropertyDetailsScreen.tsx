...
export function PropertyDetailsScreen({ property, initialStrategy }: PropertyDetailsScreenProps) {
  ...
  // Feature tabs state
  const [activeFeatureTab, setActiveFeatureTab] = useState<'interior' | 'exterior' | 'appliances'>(
    'interior',
  )
  ...
  const propertyFacts = [
    {
      label: 'PRICE/SQFT',
      value: property.livingArea ? `$${Math.round(property.price / property.livingArea)}` : 'N/A',
      icon: 'dollar',
    },
    {
      label: 'LOT SIZE',
      value: property.lotSize ? `${formatNumber(property.lotSize)} sqft` : 'N/A',
      subValue: property.lotSizeAcres ? `${property.lotSizeAcres} acres` : undefined,
      icon: 'grid',
    },
    ...
    { label: 'STORIES', value: property.stories || 'N/A', icon: 'layers' },
    {
      label: 'EST. VALUE',
      value:
        (property.valueIqEstimate ?? property.zestimate)
          ? formatPrice(property.valueIqEstimate ?? property.zestimate ?? 0)
          : 'N/A',
      icon: 'trending',
      highlight: true,
    },
    {
      ...
      value:
        (property.rentalIqEstimate ?? property.rentZestimate)
          ? `${formatPrice(property.rentalIqEstimate ?? property.rentZestimate ?? 0)}/mo`
          : 'N/A',
      icon: 'rent',
      highlight: true,
    },
    {
      label: 'HOA FEE',
      value: property.hoaFee ? `${formatPrice(property.hoaFee)}/mo` : 'None',
      icon: 'shield',
    },
    {
      label: 'ANNUAL TAX',
      value: property.annualTax ? formatPrice(property.annualTax) : 'N/A',
      icon: 'receipt',
    },
    {
      label: 'PARKING',
      ...
      bed: (
        <>
          <path
            ...
            d="M3 7.5V18a1.5 1.5 0 001.5 1.5h15A1.5 1.5 0 0021 18V7.5"
          />
          <path strokeLinecap="round" strokeLinejoin="round" d="M3 12h18v3H3z" />
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M5.5 12V9a2 2 0 012-2h2a2 2 0 012 2v3"
          />
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M12.5 12V9a2 2 0 012-2h2a2 2 0 012 2v3"
          />
          <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 19.5V21m15-1.5V21" />
        </>
      ),
      bath: (
        <>
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M4 12h16v5a3 3 0 01-3 3H7a3 3 0 01-3-3v-5z"
          />
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M6 12V5a2 2 0 012-2h1a2 2 0 012 2v1"
          />
  ...
  return (
    <div
      className="min-h-screen max-w-[480px] mx-auto relative font-['Inter',sans-serif]"
    ...
    >
      ...
      <main className="pb-6">
        ...
        >
          ...
          <div className="relative w-full h-60 overflow-hidden">
            ...
            {images.length > 1 && (
              <>
                <button
                  className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center shadow-md transition-colors hover:bg-white/10"
                  style={{ backgroundColor: 'rgba(12,18,32,0.85)', color: colors.text.primary }}
                ...
                <button
                  className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center shadow-md transition-colors hover:bg-white/10"
                  style={{ backgroundColor: 'rgba(12,18,32,0.85)', color: colors.text.primary }}
            ...
            {/* Image Counter */}
            <div
              className="absolute top-3 right-3 text-white px-2.5 py-1 rounded-xl text-xs font-medium flex items-center gap-1"
              style={{ backgroundColor: 'rgba(12,18,32,0.75)', fontVariantNumeric: 'tabular-nums' }}
            >
              <svg
                width="14"
                height="14"
            ...
            <div className="flex gap-1.5 px-3 py-2 overflow-x-auto">
              {images.slice(0, 12).map((img, idx) => (
                <img
                  ...
                  style={idx === currentImageIndex ? { borderColor: colors.brand.blue } : undefined}
                  src={img}
                  alt={`Thumbnail ${idx + 1}`}
        ...
        >
          ...
          {expandedSections.facts && (
            <div className="px-4 pb-4">
              ...
              {/* Divider */}
              <div className="h-px mb-3" style={{ backgroundColor: colors.ui.border }} />
                  ...
                  >
                    ...
                    <div className="flex flex-col items-end gap-0.5">
                      <span
                        ...
                        style={{ color: fact.highlight ? colors.brand.blue : colors.text.primary }}
                      >
                        {fact.value}
                      </span>
                      {fact.subValue && (
              ...
              <div className="flex gap-2 mb-3">
                {(['interior', 'exterior', 'appliances'] as const).map((tab) => (
                  <button
                    ...
                    style={{
                      backgroundColor:
                        activeFeatureTab === tab ? colors.brand.blueDeep : colors.background.cardUp,
                    ...
                    {currentFeatures.map((feature, idx) => (
                      ...
                      >
                        <span
                          ...
                          style={{ backgroundColor: colors.status.positive }}
                        />
                        {feature}
                      </li>
                    ))}
        ...
        {/* Location Accordion */}
        <div className="overflow-hidden" style={{ backgroundColor: colors.background.card }}>
          <div
            className="flex items-center gap-3 px-4 py-3.5 cursor-pointer hover:bg-white/5 transition-colors"
