...
const NavIcons: Record<string, (isActive: boolean) => JSX.Element> = {
  search: (isActive) => (
    <svg
      ...
      height="18"
      fill="none"
      stroke={isActive ? 'var(--accent-sky)' : 'var(--text-muted)'}
      strokeWidth="1.5"
      viewBox="0 0 24 24"
    >
      <path
...
const FactIcons: Record<string, JSX.Element> = {
  ...
  bath: (
    <>
      ...
      />
      <path strokeLinecap="round" strokeLinejoin="round" d="M6 12V5a2 2 0 012-2h1a2 2 0 012 2v1" />
...
export function PropertyDetailsPage({
  ...
  const propertyFacts = useMemo(
    () => [
      ...
      {
        label: 'LOT SIZE',
        value: property.lotSize || '9,091 sqft',
        subValue: property.lotAcres || '0.21 acres',
        icon: 'grid',
      },
      ...
      },
      {
        ...
        value: `${formatPrice(property.rent || 0)}/mo`,
        icon: 'rent',
        highlight: true,
      },
      ...
      {
        label: 'ANNUAL TAX',
        value: property.annualTax ? formatPrice(property.annualTax) : '$9,980',
        icon: 'receipt',
      },
    ...
    <div className="flex flex-col h-full bg-[var(--surface-base)] font-sans relative">
      ...
      <header className="sticky top-0 z-50">
        ...
        <div className="bg-[var(--surface-card)] px-4 py-3 relative">
          ...
          {/* Property Accordion */}
          <div
            className={`overflow-hidden transition-all duration-300 ${isPropertyOpen ? 'max-h-52' : 'max-h-0'}`}
          >
            <div className="flex gap-3 pt-3 pb-1 border-t border-white/10 mt-2">
              ...
              <div className="grid grid-cols-3 gap-x-3 gap-y-1.5 flex-1">
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] font-semibold text-[var(--text-inverse)] bg-[var(--accent-sky)] px-1.5 py-0.5 rounded inline-block w-fit">
                    For Sale
                  </span>
                  <span className="text-[9px] text-slate-500 uppercase tracking-wide mt-0.5">
                    Status
                  </span>
                </div>
              </div>
            ...
            <div className="absolute top-full left-4 right-4 bg-[var(--surface-card)] rounded-xl shadow-2xl z-50 overflow-hidden mt-0">
              ...
              {STRATEGIES.map((strategy) => (
                ...
                >
                  ...
                  {strategy.short === currentStrategy && (
                    <svg
                      className="w-4 h-4 text-[var(--accent-sky)]"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
      ...
      <main className="flex-1 overflow-y-auto pb-24">
            ...
            {property.images?.length ? (
              <>
                ...
                <button
                  ...
                  onClick={() =>
                    setCurrentImageIndex((i) => Math.min(property.images!.length - 1, i + 1))
                  }
                >
                  <svg
                    width="16"
                ...
                <img
                  ...
                  className={`w-12 h-9 rounded-md object-cover cursor-pointer flex-shrink-0 border-2 transition-all ${
                    idx === currentImageIndex
                      ? 'opacity-100 border-[var(--accent-sky)]'
        ...
        <div
          className={`bg-[var(--surface-card)] rounded-xl mx-4 mb-2.5 border border-[var(--border-subtle)] shadow-sm overflow-hidden ${expandedSections.facts ? 'open' : ''}`}
        >
          <div
          ...
          >
            <svg
              className="w-6 h-6 text-[var(--accent-sky)]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <span className="flex-1 text-sm font-semibold text-[var(--text-heading)] uppercase tracking-wide">
              Property Facts
            </span>
            <svg
                ...
                {propertyFacts.map((fact, idx) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col items-end gap-0.5">
                      <span
                        className={`text-sm font-semibold tabular-nums ${fact.highlight ? 'text-[var(--accent-sky)]' : 'text-[var(--text-heading)]'}`}
                      >
                        {fact.value}
                      </span>
                      {fact.subValue && (
        ...
        <div
          className={`bg-[var(--surface-card)] rounded-xl mx-4 mb-2.5 border border-[var(--border-subtle)] shadow-sm overflow-hidden ${expandedSections.features ? 'open' : ''}`}
        >
          <div
            className="flex items-center gap-3 px-4 py-3.5 cursor-pointer hover:bg-slate-50 transition-colors"
        ...
        {/* Location Accordion */}
        <div
          className={`bg-[var(--surface-card)] rounded-xl mx-4 mb-2.5 border border-[var(--border-subtle)] shadow-sm overflow-hidden ${expandedSections.location ? 'open' : ''}`}
        >
          <div
            className="flex items-center gap-3 px-4 py-3.5 cursor-pointer hover:bg-slate-50 transition-colors"
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-[var(--surface-card)] px-4 py-3 flex items-center justify-between gap-2 border-t border-[var(--border-default)] shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
        <button
          className="flex flex-col items-center gap-0.5 px-2.5 py-1.5 text-slate-500 hover:text-[var(--accent-sky)] transition-colors"
          onClick={onBack}
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
