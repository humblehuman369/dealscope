...
export function getGradeColors(grade: DealGrade | ProfitGrade): {
  primary: string
  glow: string
  bg: string
} {
...
export function ScoreBadge({ type, score, grade, size = 'medium' }: ScoreBadgeProps) {
  ...
  const colors = isScoreType ? getScoreColors(score ?? 0) : getGradeColors(grade ?? 'B')
