...
// MAIN COMPONENT
// ============================================
export function StrWorksheet({ property, propertyId, onExportPDF }: StrWorksheetProps) {
  ...
  const { result: dealScoreResult, isLoading: isDealScoreLoading } = useDealScore({
    ...
    insurance: inputs.insurance_annual ?? originalPrice * OPERATING_INSURANCE_PCT,
    vacancyRate: 1 - (inputs.occupancy_rate || 0.65), // Convert occupancy to vacancy
    maintenancePct: 0.05,
    managementPct: 0.1, // STR management typically 10%
    downPaymentPct: inputs.down_payment_pct || 0.2,
  ...
  const MetricRow = ({
    ...
    <div className="flex items-center justify-between py-2.5 border-b border-slate-100 last:border-0">
      ...
      <div className="flex items-center gap-2">
        <span
          className={`text-sm font-semibold tabular-nums ${isGood ? 'text-teal' : 'text-slate-800'}`}
        >
          {value}
        </span>
  ...
  const estLTRCashFlow =
    estLTRMonthlyRent -
    calc.monthlyPayment -
    inputs.supplies_monthly * 0.2 -
    (inputs.insurance_annual / 12) * 0.6 -
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 sm:gap-3">
            <div className="rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 bg-teal/10">
              ...
              <div className="text-xs sm:text-sm lg:text-base font-bold text-teal tabular-nums truncate">
                {fmt.currencyCompact(calc.grossRevenue)}
              </div>
            </div>
            ...
            <div
              className={`rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 ${isProfit ? 'bg-teal/10' : 'bg-red-500/10'}`}
            >
              <div className="text-[8px] sm:text-[9px] lg:text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-0.5 sm:mb-1 truncate">
                Annual Profit
              </div>
              <div
                className={`text-xs sm:text-sm lg:text-base font-bold tabular-nums truncate ${isProfit ? 'text-teal' : 'text-red-500'}`}
              >
                {isProfit ? '+' : ''}
                {fmt.currencyCompact(calc.annualCashFlow)}
              </div>
            </div>
            <div className="bg-slate-50 border border-slate-200 rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0">
              ...
              <div className="text-xs sm:text-sm lg:text-base font-bold text-slate-800 tabular-nums">
            ...
            <div
              className={`rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 ${
                dealScore >= 70
                  ? 'bg-teal/15'
                  : dealScore >= 40
                    ? 'bg-amber-500/10'
            ...
            >
              ...
              <div
                className={`text-xs sm:text-sm lg:text-base font-bold tabular-nums ${
                  dealScore >= 70
                    ? 'text-teal'
                    : dealScore >= 40
                      ? 'text-amber-500'
              ...
              <div className="space-y-3">
                ...
                <Section index={2} title="Revenue" iconKey="calendar">
                  ...
                  <div className="mt-3 pt-3 space-y-2">
                    <SummaryBox
                      label="Gross Revenue"
                      value={`${fmt.currency(calc.grossRevenue)}/yr`}
                      variant="teal"
                    />
                  </div>
                </Section>
                ...
                <Section index={3} title="Expenses" iconKey="expense">
                  ...
                  <div className="mt-3 pt-3">
                    <SummaryBox
                      ...
                      value={`${fmt.currency(calc.grossExpenses)}/yr`}
                      variant="danger"
                    />
                  </div>
                </Section>
                ...
                <Section index={4} title="Cash Flow" iconKey="cashflow">
                  ...
                  <DisplayRow
                    ...
                    value={`−${fmt.currency(calc.grossExpenses)}`}
                    variant="danger"
                  />
                  <DisplayRow label="Net Operating Income" value={fmt.currency(calc.noi)} />
                  <DisplayRow
                    label="Annual Debt Service"
                    value={`−${fmt.currency(calc.annualDebtService)}`}
                    variant="danger"
                  />
                  <div className="mt-3 pt-3 space-y-2">
                    <SummaryBox
                      label="Monthly Cash Flow"
                      value={fmt.currency(calc.monthlyCashFlow)}
                      variant={calc.monthlyCashFlow >= 0 ? 'success' : 'danger'}
                    />
                    <SummaryBox
                ...
                <Section index={5} title="Returns" iconKey="returns">
                  <MetricRow
                    label="Cap Rate"
                    value={`${calc.capRate.toFixed(1)}%`}
                    target="8%"
                    isGood={calc.capRate >= 8}
                  />
                  <MetricRow
                    label="Cash on Cash"
                    value={`${calc.cashOnCash.toFixed(1)}%`}
                    target="10%"
                    isGood={calc.cashOnCash >= 10}
                  />
                  <MetricRow
                    label="DSCR"
                    value={fmt.ratio(calc.dscr)}
                    target="1.2x"
                    isGood={calc.dscr >= 1.2}
                  />
                  <MetricRow
                    label="Gross Rent Multiplier"
                    value={calc.grm.toFixed(1)}
                    target="10"
                    isGood={calc.grm <= 10 && calc.grm > 0}
                  />
              ...
              <div className="sm:sticky sm:top-28 space-y-4 sm:max-h-[calc(100vh-8rem)] sm:overflow-y-auto">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 overflow-hidden">
                  ...
                  <div className="px-5 py-4 border-t border-slate-100">
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-800 mb-3">
                      RETURNS VS TARGETS
                    </div>
                    {targets.map((t, i) => (
                      <div
                      ...
                      >
                        ...
                        <span
                          className={`text-sm font-semibold tabular-nums ${t.met ? 'text-teal' : 'text-slate-800'}`}
                        >
                          {t.actual.toFixed(1)}
                          {t.unit} / {t.target}
                          {t.unit}
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 p-5">
                  <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-800 mb-4">
                    PRICE POSITION
                  </div>
                    ...
                    <div className="relative">
                      <svg width="160" height="90" viewBox="0 0 160 90">
                        ...
                        <line
                          ...
                          x2={needleX}
                          y2={needleY}
                          stroke={dealScore >= 50 ? 'var(--accent-sky)' : '#EF4444'}
                          strokeWidth="3"
                          strokeLinecap="round"
                      ...
                      <div className="absolute bottom-0 left-1 text-[9px] font-bold text-red-500">
                        LOSS
                      </div>
                      <div className="absolute bottom-0 right-1 text-[9px] font-bold text-teal">
                        PROFIT
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      <span
                        className={`text-sm font-medium ${inputs.purchase_price <= incomeValue ? 'text-teal' : 'text-red-500'}`}
                      >
                        Your Price
                      </span>
                      <span
                        className={`text-sm font-bold tabular-nums ${inputs.purchase_price <= incomeValue ? 'text-teal' : 'text-red-500'}`}
                      >
                    ...
                    {inputs.purchase_price > incomeValue && (
                      <div className="mt-2 px-3 py-2 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                        <p className="text-xs text-amber-600 font-medium">
                          Price is {fmt.currency(inputs.purchase_price - incomeValue)} above income
                          value
                        </p>
                      </div>
                    )}
