...
export function buildSTRMetricsData(calc: {
  ...
  const breakevenOccupancy =
    calc.breakevenOccupancy ??
    (calc.grossRevenue > 0
      ? ((calc.grossExpenses + calc.annualDebtService) / calc.grossRevenue) *
        calc.occupancyRate *
        100
      : 0)
...
export function STRMetricsChart({ data }: STRMetricsChartProps) {
  ...
  const verdict: VerdictData = useMemo(() => {
    ...
    if (score >= 80)
      return { label: 'STRONG', detail: '(Meets 4+ critical checks)', status: 'good' }
    if (score >= 60)
      return { label: 'MODERATE', detail: '(Meets 3 critical checks)', status: 'warn' }
  ...
  const kpis: KPI[] = [
    ...
    {
      label: 'Occupancy',
      value: fmt.percent(data.occupancyRate * 100),
      hint: 'Demand / fill rate',
    },
  ...
  const factors: Factor[] = useMemo(
    () => [
      ...
      },
      {
        ...
        status:
          data.grossExpenses / data.grossRevenue <= 0.45
            ? 'good'
            : data.grossExpenses / data.grossRevenue <= 0.55
              ? 'warn'
              : 'bad',
        tag:
          data.grossExpenses / data.grossRevenue <= 0.45
  ...
  const returnsPanel: BarItem[] = [
    ...
    },
    {
      ...
      meter: Math.min(100, data.cashOnCash * 8),
    },
    {
      label: 'Monthly Cash Flow',
      value: fmt.currency(data.monthlyCashFlow),
      meter: Math.min(100, (data.monthlyCashFlow / 2000) * 100),
    },
  ]
  ...
  const entryPanel: BarItem[] = [
    {
      ...
      meter: Math.min(100, data.buyDiscount * 5),
    },
    {
      label: 'Total Cash Req.',
      value: fmt.currencyK(data.totalCashRequired),
      meter: Math.min(100, 100 - data.totalCashRequired / 2000),
    },
    ...(data.furnishingSetup
      ? [{ label: 'Furn. Setup', value: fmt.currencyK(data.furnishingSetup), meter: 60 }]
      : []),
  ]
  ...
  const financialSnapshot: BarItem[] = [
    ...
    {
      label: 'Total OpEx',
      value: fmt.currency(data.grossExpenses),
      meter: Math.min(100, (data.grossExpenses / data.grossRevenue) * 100),
    },
    {
      label: 'NOI',
      value: fmt.currency(data.noi),
      meter: Math.min(100, (data.noi / data.grossRevenue) * 100),
    },
    { label: 'Annual Debt Service', value: fmt.currency(data.annualDebtService), meter: 50 },
    {
      label: 'Annual Cash Flow',
      value: fmt.currency(data.annualCashFlow),
      meter: data.annualCashFlow > 0 ? 65 : 20,
    },
  ]
  ...
  const footerStats: MetaItem[] = [
    {
      label: 'ADR • Occupancy • RevPAN',
      value: `${fmt.currency(data.adr)} • ${fmt.percent(data.occupancyRate * 100)} • ${fmt.currency(data.revPAN)}`,
    },
  ]

  // Seasonality (mock data if not provided)
  const seasonalityMonths = ['J', 'F', 'M', 'A', 'M', 'J', 'J', 'A', 'S', 'O', 'N', 'D']
  const seasonalityHeights = data.seasonalityHeights || [
    45, 50, 58, 65, 72, 82, 92, 88, 75, 62, 52, 48,
  ]
          ...
          <div className="mt-6">
            ...
            <div className="space-y-1">
              {financialSnapshot.map((bar) => (
                <div
                  key={bar.label}
                  className="grid grid-cols-[1fr_2fr_auto] gap-3 items-center py-2"
                >
