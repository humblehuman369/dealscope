...
export function PropertyStatusPills({
  ...
  const isActivelyListed =
    !isOffMarket && (listingStatus === 'FOR_SALE' || listingStatus === 'FOR_RENT')
    ...
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      <div
        className={`rounded-lg px-4 py-2 ${
          isActiveListing ? 'bg-[rgba(8,145,178,0.12)]' : 'bg-slate-100'
        }`}
      >
      ...
      <div
        className={`rounded-lg px-4 py-2 ${
          isWarningSellerType ? 'bg-[rgba(245,158,11,0.12)]' : 'bg-slate-100'
        }`}
      >
        <div className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide">
          OWNER
