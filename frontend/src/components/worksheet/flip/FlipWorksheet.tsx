...
// MAIN COMPONENT
// ============================================
export function FlipWorksheet({ property, propertyId, onExportPDF }: FlipWorksheetProps) {
  ...
  if (allInDiscount <= 5) {
    ...
    verdictSub = 'Excellent deal - great all-in cost'
  } else if (allInDiscount <= 10) {
    verdict = 'Good Opportunity'
    verdictSub = 'Very good flip fundamentals'
  ...
  const targets = [
    ...
    {
      label: 'Profit Margin',
      actual: calc.profitMargin,
      target: 10,
      unit: '%',
      met: calc.profitMargin >= 10,
    },
    { label: 'All-In/ARV', actual: calc.allInPctArv, target: 70, unit: '%', met: calc.meets70Rule },
    {
      label: 'Net Profit',
      actual: calc.actualProfit,
      target: 30000,
      unit: '$',
      met: calc.actualProfit >= 30000,
    },
  ]
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 sm:gap-3">
            ...
            <div className="rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 bg-slate-800/5">
              ...
              <div className="text-xs sm:text-sm lg:text-base font-bold text-slate-800 tabular-nums truncate">
                {fmt.currencyCompact(calc.cashToClose + rehabCosts)}
              </div>
            </div>
            <div
            ...
            >
              ...
              >
                {isProfit ? '+' : ''}
                {fmt.currencyCompact(calc.actualProfit)}
              </div>
            </div>
            <div className="rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 bg-blue-500/10">
              <div className="text-[8px] sm:text-[9px] lg:text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-0.5 sm:mb-1 truncate">
            ...
            <div className="grid grid-cols-[1.4fr,1.2fr] md:grid-cols-[1.5fr,1.2fr] lg:grid-cols-[1.2fr,1fr] gap-4 sm:gap-6 items-start">
              ...
              <div className="space-y-3">
                ...
                <Section index={2} title="Valuation" iconKey="tool">
                  ...
                  <div className="mt-3 pt-3">
                    <SummaryBox
                      label={calc.meets70Rule ? '✓ Meets 70% Rule' : '✗ Above 70% Rule'}
                      value={fmt.currency(purchasePrice - calc.mao)}
                      variant={calc.meets70Rule ? 'success' : 'danger'}
                    />
                  </div>
                </Section>
                ...
                <Section index={3} title="Holding Costs" iconKey="clock">
                  ...
                  <DisplayRow
                    ...
                    value={`−${fmt.currency(calc.monthlyInterest)}`}
                    variant="danger"
                  />
                  <InputRow
                ...
                <Section index={4} title="Sale Proceeds" iconKey="sale">
                  ...
                  <InputRow
                    ...
                    step={0.5}
                    format="percent"
                    subValue={fmt.currency(calc.sellingCosts)}
                  />
                  ...
                  <div className="mt-3 pt-3">
                    <SummaryBox
                      ...
                      value={fmt.currency(calc.netSaleProceeds)}
                      variant="success"
                    />
                  </div>
                </Section>
                ...
                <Section index={5} title="Returns" iconKey="returns">
                  ...
                  <DisplayRow
                    label="Net Profit"
                    value={fmt.currency(calc.actualProfit)}
                    variant={isProfit ? 'success' : 'danger'}
                  />
                  <DisplayRow
                    label="ROI"
                    value={fmt.percent(calc.roi)}
                    variant={calc.roi >= 20 ? 'success' : 'default'}
                  />
                  <DisplayRow
                    label="Annualized ROI"
                    value={fmt.percent(calc.annualizedRoi)}
                    variant={calc.annualizedRoi >= 50 ? 'success' : 'default'}
                  />
                  <DisplayRow
                    label="Profit Margin"
                    value={fmt.percent(calc.profitMargin)}
                    variant={calc.profitMargin >= 10 ? 'success' : 'default'}
                  />
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 p-5">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div className="flex justify-between items-center py-2 px-3 bg-slate-50 rounded-lg">
                      ...
                      <span
                        className={`font-semibold ${calc.meets70Rule ? 'text-emerald-600' : 'text-red-500'}`}
                      >
                        {fmt.currencyCompact(calc.allInCost)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center py-2 px-3 bg-slate-50 rounded-lg">
                      ...
                      <span className="font-semibold text-slate-800">
                        {fmt.currencyCompact(calc.mao)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center py-2 px-3 bg-slate-50 rounded-lg">
                      <span className="text-slate-500">Income Value</span>
                      <span className="font-semibold text-slate-800">
                        {fmt.currencyCompact(calc.incomeValue)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 p-5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="py-3 px-4 rounded-lg bg-blue-50 border border-blue-200">
                      ...
                      <div
                        className={`text-lg font-bold ${isProfit ? 'text-blue-600' : 'text-red-500'}`}
                      >
                        {fmt.currencyCompact(calc.actualProfit)}
                      </div>
                    </div>
                    <div className="py-3 px-4 rounded-lg bg-slate-50">
                      <div className="text-xs text-slate-500 mb-1">Cash Invested</div>
                      <div className="text-lg font-bold text-slate-800">
                        {fmt.currencyCompact(calc.totalCashInvested)}
                      </div>
                    </div>
                    <div className="py-3 px-4 rounded-lg bg-slate-50">
                      <div className="text-xs text-slate-500 mb-1">ROI</div>
                      <div
                        className={`text-lg font-bold ${calc.roi >= 20 ? 'text-blue-600' : 'text-slate-800'}`}
                      >
                        {fmt.percent(calc.roi)}
                      </div>
                    </div>
                    <div className="py-3 px-4 rounded-lg bg-slate-50">
                      <div className="text-xs text-slate-500 mb-1">Annualized</div>
                      <div
                        className={`text-lg font-bold ${calc.annualizedRoi >= 50 ? 'text-blue-600' : 'text-slate-800'}`}
                      >
                        {fmt.percent(calc.annualizedRoi)}
                      </div>
                    </div>
                  </div>
                </div>
