  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-6 gap-3">
        <KPIBox label={priceLabel} value={formatCurrency(purchasePrice)} variant="teal" />
        <KPIBox label="Cash Needed" value={formatCurrency(cashNeeded)} variant="navy" />
        <KPIBox
          label="Annual Profit"
          value={`${isProfit ? '+' : ''}${formatCurrency(cashFlow)}`}
          variant={isProfit ? 'teal' : 'danger'}
        />
        <KPIBox label="Cap Rate" value={formatPercent(capRate)} variant="neutral" />
        <KPIBox label="CoC Return" value={formatPercent(cocReturn)} variant="neutral" />
