      ...
      <tbody>
        {rows.map((row) => (
          <tr key={row.metric} className="border-t border-slate-200 dark:border-slate-700">
            ...
            <td className="py-3 tabular-nums text-slate-800 dark:text-white font-semibold">
              {row.result}
            </td>
            <td className="py-3">
              <span
                className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium ${getStatusBgColor(row.status)} ${getStatusColor(row.status)}`}
              >
...
export function LTRMetricsChart({ data, theme = 'light' }: LTRMetricsChartProps) {
    ...
    () => [
      {
        ...
        label:
          data.capRate >= 8 ? 'Above Target' : data.capRate >= 5 ? 'Acceptable' : 'Below Target',
      },
      {
        metric: 'Cash-on-Cash',
        ...
        ? [
            {
              ...
              status: (data.irr >= 15 ? 'good' : data.irr >= 10 ? 'warn' : 'bad') as
                | 'good'
                | 'warn'
                | 'bad',
              label:
                data.irr >= 15 ? 'Value-Add Tier' : data.irr >= 10 ? 'Stabilized' : 'Below Target',
            },
          ]
        : []),
        ...
        ? [
            {
              ...
              status: (data.equityCapture >= 10
                ? 'good'
                : data.equityCapture >= 5
                  ? 'warn'
                  : 'bad') as 'good' | 'warn' | 'bad',
              label:
                data.equityCapture >= 10
                  ? 'Discounted Entry'
                  : data.equityCapture >= 5
                    ? 'Fair Value'
                    : 'Premium',
  ...
  const riskMetrics: MetricRow[] = useMemo(
    () => [
      ...
      },
      {
        ...
        status:
          data.breakevenOccupancy <= 75 ? 'good' : data.breakevenOccupancy <= 85 ? 'warn' : 'bad',
        label:
          data.breakevenOccupancy <= 75
            ? 'Defensive'
            : data.breakevenOccupancy <= 85
              ? 'Standard'
              : 'Tight',
      },
    ],
  ...
  const signals = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        tag:
          data.expenseRatio <= 40 ? 'Efficient' : data.expenseRatio <= 50 ? 'Watch' : 'High Cost',
      },
      {
  ...
  return (
    <div className={`rounded-xl overflow-hidden ${isDark ? 'bg-slate-900' : 'bg-white'}`}>
      {/* Header */}
      <div
        className={`px-6 py-4 border-b ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6">
        {/* LEFT: Decision Panel */}
        <div
          className={`rounded-xl border ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-white border-slate-200'} overflow-hidden`}
        >
          <div className={`px-5 py-3 border-b ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
            <div className="flex items-center justify-between">
              ...
              <span
                className={`text-xs px-2 py-1 rounded-full ${isDark ? 'bg-slate-700 text-slate-300' : 'bg-slate-100 text-slate-600'}`}
              >
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="flex justify-center">
              <div
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${getStatusBgColor(data.verdict.status)} animate-pulse`}
              >
                <span
                  className={`w-2 h-2 rounded-full ${getStatusDotColor(data.verdict.status)}`}
                />
                <span className={`font-bold ${getStatusColor(data.verdict.status)}`}>
                  {data.verdict.label}
                </span>
            ...
            <div className="flex items-center gap-6 justify-center">
              ...
              <div>
                ...
                <p
                  className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'} max-w-[180px]`}
                >
            ...
            <div className={`p-4 rounded-lg ${isDark ? 'bg-slate-700/30' : 'bg-slate-50'}`}>
              <p className={`text-sm ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                <strong className={isDark ? 'text-white' : 'text-slate-900'}>Bottom Line:</strong>{' '}
                {data.bottomLine}
              </p>
            </div>
        ...
        >
          ...
          <div className="p-5 space-y-6">
            ...
            <div className="space-y-2">
              {signals.map((signal) => (
                ...
                >
                  ...
                  <span
                    className={`text-sm font-medium px-2 py-0.5 rounded ${isDark ? 'bg-slate-600 text-slate-200' : 'bg-slate-200 text-slate-700'}`}
                  >
                    {signal.tag}
                  </span>
                </div>
            ...
            <div>
              <h4
                className={`text-sm font-semibold mb-4 ${isDark ? 'text-white' : 'text-slate-900'}`}
              >
                Performance Bars
              </h4>
                ...
                {bars.map((bar) => (
                  <div key={bar.label} className="space-y-1">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span
                        className={`font-medium tabular-nums ${isDark ? 'text-white' : 'text-slate-900'}`}
                      >
                        {bar.value}%
                      </span>
                    </div>
            ...
            <div className={`p-4 rounded-lg ${isDark ? 'bg-slate-700/30' : 'bg-slate-50'}`}>
              <p className={`text-sm ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                ...
                {score}% composite score across returns and risk protection.
              </p>
            </div>
          </div>
        </div>
...
export function buildLTRMetricsData(calc: {
  ...
  const expenseRatio = calc.grossIncome > 0 ? (calc.grossExpenses / calc.grossIncome) * 100 : 0

  const cashFlowYield =
    calc.totalCashNeeded && calc.totalCashNeeded > 0
  ...
  const strategyFit =
    calc.cashOnCash >= 8 ? 'Value-Add' : calc.cashOnCash >= 5 ? 'Stabilized' : 'Development'
  const riskLevel = calc.dscr >= 1.5 ? 'Low' : calc.dscr >= 1.25 ? 'Moderate' : 'High'
  const returnProfile =
    calc.capRatePurchase >= 8
      ? 'Yield + Growth'
      : calc.capRatePurchase >= 5
        ? 'Yield Focused'
        : 'Growth Focused'
  const downsideProtection =
    calc.breakEvenRatio <= 70 ? 'Strong' : calc.breakEvenRatio <= 80 ? 'Moderate' : 'Weak'
  ...
  return {
    ...
    equityCapture:
      calc.equityAtPurchase && calc.totalCashNeeded
