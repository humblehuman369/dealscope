...
export function BrrrrWorksheet({ property, propertyId, onExportPDF }: BrrrrWorksheetProps) {
  ...
  const initialPurchasePrice = calculateInitialPurchasePrice({
    ...
    maintenancePct: 0.05,
    managementPct: 0,
    downPaymentPct: 0.1, // BRRRR typically uses hard money
  ...
  useEffect(() => {
    ...
    if (Number.isFinite(b?.refinance_interest_rate))
      setRefiInterestRate(b.refinance_interest_rate * 100)
    ...
    if (Number.isFinite(b?.refinance_ltv) && Number.isFinite(b?.refinance_closing_costs_pct)) {
  ...
  const { result: dealScoreResult } = useDealScore({
    ...
    downPaymentPct: 0.1, // BRRRR typically uses 10% down with hard money
  ...
  const calc = useMemo(() => {
    ...
    const totalHoldingCosts =
      (monthlyInterest + monthlyTaxes + monthlyInsurance + utilities) * holdingMonths
    ...
    const cashRecoveryPct =
      allInCost > 0 ? ((refinanceLoanAmount - allInCost) / allInCost) * 100 : 0
    ...
    const refiMonthlyPayment =
      refinanceLoanAmount > 0
        ? (refinanceLoanAmount *
            (refiMonthlyRate * Math.pow(1 + refiMonthlyRate, refiNumPayments))) /
    ...
    const cashOnCash =
      cashLeftInDeal > 0 ? (annualCashFlow / cashLeftInDeal) * 100 : annualCashFlow > 0 ? 999 : 0
          ...
          <div className="text-right">
            {isEditing ? (
              <input
                ...
                autoFocus
                className="w-24 px-2 py-0.5 text-sm font-semibold text-slate-800 tabular-nums text-right border border-blue-500 rounded focus:outline-none focus:ring-1 focus:ring-blue-500"
              />
            ) : (
              <span
                onClick={handleStartEdit}
                className="text-sm font-semibold text-slate-800 tabular-nums cursor-pointer hover:text-blue-600 hover:underline"
              >
                {displayValue}
              </span>
            )}
            {subValue && (
              <span className="text-xs text-slate-400 ml-1.5 tabular-nums">{subValue}</span>
            )}
          </div>
        </div>
  ...
  const DisplayRow = ({
    ...
    label: string
    value: string
    variant?: 'default' | 'success' | 'danger' | 'muted' | 'blue'
  }) => {
    const colors = {
  ...
  const SummaryBox = ({
    ...
    return (
      <div className={`flex items-center justify-between p-3 rounded-lg border ${styles[variant]}`}>
        <span className="text-sm font-medium text-slate-600">{label}</span>
        <span className={`text-base font-bold tabular-nums ${textColor[variant]}`}>{value}</span>
      </div>
    )
  }
    ...
    return (
      <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 overflow-hidden scroll-mt-[72px]">
        <button
          ...
          className="w-full flex items-center justify-between p-4 min-h-[44px] hover:bg-slate-50 transition-colors"
        >
          <div className="flex items-center gap-3">
            <div
              className={`w-8 h-8 rounded-lg flex items-center justify-center ${isComplete ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-500'}`}
            >
              {Icons[iconKey]}
            </div>
            <span className="font-semibold text-slate-800">{title}</span>
            {badge && (
              <span className="text-[9px] font-semibold uppercase tracking-wide bg-blue-100 text-blue-600 px-1.5 py-0.5 rounded">
                {badge}
              </span>
            )}
          </div>
  ...
  if (allInDiscount <= 5) {
    ...
    verdictSub = infiniteCoC
      ? 'Infinite returns - all cash recovered!'
      : 'Excellent deal - great all-in cost'
  } else if (allInDiscount <= 10) {
    verdict = 'Good Opportunity'
    verdictSub = 'Very good BRRRR fundamentals'
  } else if (allInDiscount <= 15) {
    verdict = 'Moderate Opportunity'
    verdictSub = 'Good potential - negotiate firmly'
  } else if (allInDiscount <= 25) {
    verdict = 'Marginal Opportunity'
    verdictSub = 'Possible deal - need better purchase price'
  } else if (allInDiscount <= 35) {
    verdict = 'Unlikely Opportunity'
    verdictSub = 'Challenging - major price reduction needed'
  } else {
    verdict = 'Pass'
  ...
  const targets = [
    {
      label: 'All-In/ARV',
      actual: calc.allInPctArv,
      target: 75,
      unit: '%',
      met: calc.allInPctArv <= 75,
      inverse: true,
    },
    {
      label: 'Cash Recovery',
      actual: calc.cashRecoveryPct,
      target: 100,
      unit: '%',
      met: calc.cashRecoveryPct >= 100,
    },
    { label: 'Cap Rate', actual: calc.capRate, target: 8, unit: '%', met: calc.capRate >= 8 },
    {
      label: 'CoC Return',
      actual: infiniteCoC ? 999 : calc.cashOnCash,
      target: 10,
      unit: '%',
      met: calc.cashOnCash >= 10 || infiniteCoC,
    },
  ]
            ...
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="hidden sm:block flex-shrink-0 w-16 h-12 rounded-lg border border-slate-200 bg-slate-100 overflow-hidden">
                {property.property_data_snapshot?.photos?.[0] ? (
                  <img
                    src={property.property_data_snapshot.photos[0]}
                    alt="Property"
            ...
            <div className="hidden md:flex items-center gap-5 flex-shrink-0">
              <div className="text-right">
                <div className="text-xl font-bold text-slate-900">
                  {fmt.currency(
                    property.property_data_snapshot?.zestimate ||
              ...
              {property.property_data_snapshot?.isForeclosure &&
                !property.property_data_snapshot?.isBankOwned && (
              ...
              {(() => {
                ...
                return (
                  <span
                    className={`px-2.5 py-1 text-xs font-semibold uppercase tracking-wide border-2 ${config.border} ${config.text} rounded`}
                  >
                    {config.label}
                  </span>
                )
      ...
      <div className="w-full sticky top-12 z-40 bg-white border-b border-slate-200">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
          <WorksheetTabNav
            ...
            strategy="brrrr"
            zpid={property.zpid || propertyData.zpid}
          />
        </div>
      </div>
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 sm:gap-3">
            <div className="rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 bg-slate-800/5">
              <div className="text-[8px] sm:text-[9px] lg:text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-0.5 sm:mb-1 truncate">
                All-In Cost
              </div>
              <div className="text-xs sm:text-sm lg:text-base font-bold text-slate-800 tabular-nums truncate">
                {fmt.currencyCompact(calc.allInCost)}
              </div>
            </div>
            <div className="rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 bg-blue-500/10">
              <div className="text-[8px] sm:text-[9px] lg:text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-0.5 sm:mb-1 truncate">
                ARV
              </div>
              <div className="text-xs sm:text-sm lg:text-base font-bold text-blue-600 tabular-nums truncate">
                {fmt.currencyCompact(arv)}
              </div>
            </div>
            <div
              className={`rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 ${calc.cashOut > 0 ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}
            >
              <div className="text-[8px] sm:text-[9px] lg:text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-0.5 sm:mb-1 truncate">
                Cash Out
              </div>
              <div
                className={`text-xs sm:text-sm lg:text-base font-bold tabular-nums truncate ${calc.cashOut > 0 ? 'text-emerald-600' : 'text-red-500'}`}
              >
                {fmt.currencyCompact(calc.cashOut)}
              </div>
            </div>
            <div
              className={`rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 ${isProfit ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}
            >
              <div className="text-[8px] sm:text-[9px] lg:text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-0.5 sm:mb-1 truncate">
                Cash Flow
              </div>
              <div
                className={`text-xs sm:text-sm lg:text-base font-bold tabular-nums truncate ${isProfit ? 'text-emerald-600' : 'text-red-500'}`}
              >
            ...
            <div
              className={`rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 ${dealScore >= 70 ? 'bg-blue-500/15' : dealScore >= 40 ? 'bg-amber-500/10' : 'bg-red-500/10'}`}
            >
              <div className="text-[8px] sm:text-[9px] lg:text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-0.5 sm:mb-1 truncate">
                Deal Score
              </div>
              <div
                className={`text-xs sm:text-sm lg:text-base font-bold tabular-nums ${dealScore >= 70 ? 'text-blue-600' : dealScore >= 40 ? 'text-amber-500' : 'text-red-500'}`}
              >
                {dealScore}
              </div>
            </div>
              ...
              <div className="space-y-3">
                <Section index={0} title="Purchase & Rehab" iconKey="home">
                  ...
                  <InputRow
                    ...
                    step={0.5}
                    format="percent"
                    subValue={fmt.currency(calc.purchaseCosts)}
                  />
                  <div className="mt-3 pt-3">
                    <SummaryBox
                      label="Total All-In Cost"
                      value={fmt.currency(calc.allInCost)}
                      variant="blue"
                    />
                  </div>
                </Section>
                ...
                <Section index={1} title="Financing (Purchase)" iconKey="bank" badge="Hard Money">
                  ...
                  <div className="mt-3 pt-3">
                    <SummaryBox label="Cash to Close" value={fmt.currency(calc.cashToClose)} />
                  </div>
                </Section>
                ...
                <Section index={2} title="Valuation" iconKey="tool">
                  <InputRow
                    ...
                    value={arv}
                    onChange={setArv}
                    min={Math.round(listPrice * 0.7)}
                    max={Math.round(listPrice * 2)}
                    step={5000}
                    format="currency"
                  />
                  <DisplayRow
                    label="Purchase / Sq.Ft."
                    value={`$${Math.round(calc.pricePerSqft)}`}
                  />
                  <DisplayRow label="ARV / Sq.Ft." value={`$${Math.round(calc.arvPerSqft)}`} />
                  <DisplayRow
                    label="All-In % of ARV"
                    value={fmt.percent(calc.allInPctArv)}
                    variant={calc.allInPctArv <= 75 ? 'success' : 'danger'}
                  />
                  <div className="mt-3 pt-3">
                    <SummaryBox
                      label="Equity Created"
                      value={fmt.currency(calc.equityCreated)}
                      variant={calc.equityCreated > 0 ? 'success' : 'danger'}
                    />
                  </div>
                </Section>
                ...
                <Section index={3} title="Holding Costs" iconKey="clock" badge="Rehab Period">
                  ...
                  <DisplayRow
                    label="Loan Interest"
                    value={`−${fmt.currency(calc.totalHoldingInterest)}`}
                    variant="danger"
                  />
                  <InputRow
                  ...
                  <div className="mt-3 pt-3">
                    <SummaryBox
                      ...
                      value={`−${fmt.currency(calc.totalHoldingCosts)}`}
                      variant="danger"
                    />
                  </div>
                </Section>
                ...
                <Section index={4} title="Refinance" iconKey="refresh" badge="Cash-Out Refi">
                  ...
                  <DisplayRow
                    ...
                    value={fmt.currency(calc.refinanceLoanAmount)}
                    variant="blue"
                  />
                  <InputRow
                    label="Interest Rate"
                    value={refiInterestRate}
                    onChange={setRefiInterestRate}
                    min={5}
                    max={10}
                    step={0.125}
                    format="percent"
                  ...
                  />
                  <DisplayRow
                    label="Payoff Old Loan"
                    value={`−${fmt.currency(calc.loanAmount)}`}
                    variant="danger"
                  />
                  <div className="mt-3 pt-3 space-y-2">
                    <SummaryBox
                      label="Cash Out"
                      value={fmt.currency(calc.cashOut)}
                      variant={calc.cashOut > 0 ? 'success' : 'danger'}
                    />
                    <SummaryBox
                      label="Cash Left in Deal"
                      value={fmt.currency(Math.max(0, calc.cashLeftInDeal))}
                      variant={calc.cashLeftInDeal <= 0 ? 'success' : 'default'}
                    />
                  </div>
                </Section>
                ...
                <Section index={5} title="Cash Flow" iconKey="cashflow" badge="Post-Refi">
                  ...
                  <DisplayRow
                    ...
                    value={`${fmt.currency(calc.effectiveRent)}/mo`}
                    variant="success"
                  />
                  <InputRow
                    label="Property Mgmt"
                  ...
                  <DisplayRow
                    ...
                    value={`−${fmt.currency(calc.refiMonthlyPayment)}/mo`}
                    variant="danger"
                  />
                  <div className="mt-3 pt-3">
                    <SummaryBox
                      label="Monthly Cash Flow"
                      value={`${fmt.currency(calc.monthlyCashFlow)}/mo`}
                ...
                <Section index={6} title="Returns" iconKey="returns">
                  <DisplayRow
                    ...
                    value={fmt.currency(calc.totalCashInvested)}
                  />
                  <DisplayRow
                    label="Cash Recovered"
                    value={fmt.currency(calc.cashOut)}
                    variant={calc.cashOut > 0 ? 'success' : 'default'}
                  />
                  <DisplayRow
                    label="Cash Left in Deal"
                    value={fmt.currency(Math.max(0, calc.cashLeftInDeal))}
                  ...
                  <DisplayRow
                    label="Cash Recovery %"
                    value={fmt.percent(calc.cashRecoveryPct)}
                    variant={calc.cashRecoveryPct >= 100 ? 'success' : 'default'}
                  />
                  <DisplayRow
                    label="Cap Rate (on ARV)"
                    value={fmt.percent(calc.capRate)}
                    variant={calc.capRate >= 8 ? 'success' : 'default'}
                  />
                  <DisplayRow
                    label="CoC After Refi"
                    value={infiniteCoC ? '∞%' : fmt.percent(calc.cashOnCash)}
                    variant={calc.cashOnCash >= 10 || infiniteCoC ? 'success' : 'default'}
                  />
                  <DisplayRow
                    label="DSCR"
                    value={calc.dscr.toFixed(2)}
                    variant={calc.dscr >= 1.2 ? 'success' : calc.dscr >= 1 ? 'default' : 'danger'}
                  />
                  <DisplayRow
                    label="Annual Cash Flow"
                    value={fmt.currency(calc.annualCashFlow)}
                    variant={isProfit ? 'success' : 'danger'}
                  />
                </Section>
              </div>
              ...
              <div className="space-y-4 sm:sticky sm:top-28">
                <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 p-5">
                  ...
                  <div className="text-center mb-4">
                    <div
                      className={`text-base font-bold ${dealScore >= 70 ? 'text-blue-600' : dealScore >= 40 ? 'text-amber-500' : 'text-red-500'}`}
                    >
                      {verdict}
                    </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {targets.map((t, i) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-sm font-semibold tabular-nums ${t.met ? 'text-blue-600' : 'text-slate-800'}`}
                          >
                            {t.actual >= 999 ? '∞%' : fmt.percent(t.actual)}
                          </span>
                          <span className="text-[10px] text-slate-400">
                            / {t.inverse ? `≤${t.target}%` : `${t.target}%`}
                          </span>
                          {t.met ? (
                            <svg
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 p-5">
                  ...
                  <div className="relative h-3 bg-gradient-to-r from-emerald-500 via-amber-400 to-red-500 rounded-full mb-4">
                    <div
                      ...
                      style={{
                        left: `calc(${Math.min(100, Math.max(0, (calc.allInPctArv / 100) * 100))}% - 8px)`,
                      }}
                    />
                    <div
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div className="flex justify-between items-center py-2 px-3 bg-slate-50 rounded-lg">
                      <span className="text-slate-500">All-In</span>
                      <span
                        className={`font-semibold ${calc.allInPctArv <= 75 ? 'text-emerald-600' : 'text-red-500'}`}
                      >
                        {fmt.currencyCompact(calc.allInCost)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center py-2 px-3 bg-slate-50 rounded-lg">
                      <span className="text-slate-500">75% Target</span>
                      <span className="font-semibold text-slate-800">
                        {fmt.currencyCompact(arv * 0.75)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center py-2 px-3 bg-slate-50 rounded-lg">
                      <span className="text-slate-500">Equity</span>
                      <span
                        className={`font-semibold ${calc.equityCreated > 0 ? 'text-emerald-600' : 'text-red-500'}`}
                      >
                        {fmt.currencyCompact(calc.equityCreated)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 p-5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div
                      className={`py-3 px-4 rounded-lg ${calc.cashLeftInDeal <= 0 ? 'bg-emerald-50 border border-emerald-200' : 'bg-blue-50 border border-blue-200'}`}
                    >
                      <div className="text-xs text-slate-500 mb-1">Cash Left In</div>
                      <div
                        className={`text-lg font-bold ${calc.cashLeftInDeal <= 0 ? 'text-emerald-600' : 'text-blue-600'}`}
                      >
                        {fmt.currencyCompact(Math.max(0, calc.cashLeftInDeal))}
                      </div>
                    </div>
                    <div className="py-3 px-4 rounded-lg bg-slate-50">
                      <div className="text-xs text-slate-500 mb-1">Equity Captured</div>
                      <div
                        className={`text-lg font-bold ${calc.equityCreated > 0 ? 'text-emerald-600' : 'text-red-500'}`}
                      >
                        {fmt.currencyCompact(calc.equityCreated)}
                      </div>
                    </div>
                    <div className="py-3 px-4 rounded-lg bg-slate-50">
                      <div className="text-xs text-slate-500 mb-1">Cash Recovery %</div>
                      <div
                        className={`text-lg font-bold ${calc.cashRecoveryPct >= 100 ? 'text-emerald-600' : 'text-slate-800'}`}
                      >
                        {fmt.percent(calc.cashRecoveryPct)}
                      </div>
                    </div>
                    <div className="py-3 px-4 rounded-lg bg-slate-50">
                      <div className="text-xs text-slate-500 mb-1">CoC After Refi</div>
                      <div
                        className={`text-lg font-bold ${calc.cashOnCash >= 10 || infiniteCoC ? 'text-blue-600' : 'text-slate-800'}`}
                      >
                        {infiniteCoC ? '∞%' : fmt.percent(calc.cashOnCash)}
                      </div>
                    </div>
                ...
                <ProGate feature="Export PDF Report" mode="inline">
                  <button
                    onClick={onExportPDF}
                    className="w-full py-3 px-4 bg-blue-500 hover:bg-blue-600 text-white font-semibold rounded-xl transition-colors flex items-center justify-center gap-2"
                  >
                    <svg
                      className="w-5 h-5"
                      fill="none"
