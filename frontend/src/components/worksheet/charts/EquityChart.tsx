...
export function EquityChart() {
  ...
  const equityAreaPath =
    ...
    years
      .slice(1)
      .map((y, i) => `L ${getX(i + 1)} ${getY(y.totalEquity)}`)
      .join(' ') +
    // Go to bottom-right
    ...
    <div className="chart-container">
      ...
      <div className="relative h-64">
        ...
        <div className="ml-16 h-full">
          ...
          >
            {/* Equity area fill */}
            <path d={equityAreaPath} fill="rgba(4, 101, 242, 0.2)" stroke="none" />

            {/* Property value line */}
            <path d={valuePath} fill="none" stroke="var(--accent-brand-blue)" strokeWidth="0.5" />

            {/* Loan balance line */}
            <path d={loanPath} fill="none" stroke="var(--color-navy)" strokeWidth="0.5" />
