...
// ============================================
export function buildHouseHackMetricsData(
  calc: {
  ...
  unitRents?: number[],
  ownerUnit?: number,
): HouseHackMetricsData {
  const housingOffsetPct =
    calc.housingOffsetPct ??
    (calc.marketRent > 0
  ...
  const roiOnSavings = calc.totalCashNeeded > 0 ? (annualSavings / calc.totalCashNeeded) * 100 : 0
  ...
  const roomsRented =
    unitRents && ownerUnit !== undefined ? unitRents.filter((_, i) => i !== ownerUnit).length : 1
...
export function HouseHackMetricsChart({ data }: HouseHackMetricsChartProps) {
  ...
  const verdict: VerdictData = useMemo(() => {
    ...
    if (score >= 80)
      return {
        label: 'STRONG',
        detail: '(Offset 85%+ housing cost, near live-free)',
        status: 'good',
      }
    if (score >= 50)
      return { label: 'MODERATE', detail: '(Good savings, room for improvement)', status: 'warn' }
  ...
  const kpis: KPI[] = [
    {
      ...
      value: fmt.currencyMo(data.netHousingCost),
      hint: 'After room rent income',
    },
    {
      label: 'Savings vs Renting',
      value: fmt.currencyMo(data.savingsVsRenting),
      hint: 'Compared to rent baseline',
    },
    {
      label: 'Housing Cost Offset %',
      value: fmt.percent(data.housingCostOffsetPct),
      hint: 'Income ÷ housing cost',
    },
    {
      label: 'Live Free Threshold',
      value: fmt.percent(data.liveFreeThreshold),
      hint: 'Target offset to pay $0',
    },
  ]
    ...
    () => [
      {
        ...
        status:
          data.netHousingCost < data.monthlyPITI * 0.5
            ? 'good'
            : data.netHousingCost < data.monthlyPITI * 0.75
              ? 'warn'
              : 'bad',
      ...
      },
      {
        ...
        status:
          data.housingCostOffsetPct >= 80
            ? 'good'
            : data.housingCostOffsetPct >= 50
              ? 'warn'
              : 'bad',
        tag:
          data.housingCostOffsetPct >= 80
  ...
  const scenarioA: BarItem[] = [
    {
      label: `Rooms Rented • Room Rent`,
      value: `${data.roomsRented} • ${fmt.currency(data.roomRent)}`,
      meter: 72,
    },
    ...
    {
      label: 'Net Housing Cost',
      value: fmt.currency(data.netHousingCost),
      meter: data.netHousingCost <= 0 ? 90 : 58,
    },
    {
      label: 'Savings vs Renting',
      value: fmt.currency(data.savingsVsRenting),
      meter: Math.min(100, (data.savingsVsRenting / 1000) * 100),
    },
  ]
  ...
  const entrySafety: BarItem[] = [
    {
      label: 'Buy Discount %',
      value: fmt.percent(data.buyDiscount),
      meter: Math.min(100, data.buyDiscount * 10),
    },
          ...
          {data.liveFree && (
            <div className="mt-4 p-4 rounded-xl bg-emerald-50 border border-emerald-200">
              ...
              <p className="text-sm text-emerald-600 mt-1">
                Rental income covers all housing costs.
              </p>
            </div>
          )}
        </>
