...
export function HouseHackWorksheet({ property, propertyId, onExportPDF }: HouseHackWorksheetProps) {
  ...
  const calc = useMemo(() => {
    ...
    const fullRentalCashFlow =
      unitRents.reduce((a, b) => a + b, 0) * (1 - vacancyRate / 100) -
      totalExpenses -
      monthlyPayment
    ...
    const housingOffsetPct =
      marketRent > 0 ? Math.max(0, ((marketRent - actualHousingCost) / marketRent) * 100) : 0
    ...
    return {
      ...
      totalExpenses,
      totalPITI,
      yourHousingCost: actualHousingCost,
      savingsVsRenting,
      monthlyCashFlow,
      fullRentalCashFlow,
      annualCashFlow,
  ...
  const housingCostPercent =
    marketRent > 0 ? Math.max(0, (calc.yourHousingCost / marketRent) * 100) : 0
  let verdict: string, verdictSub: string
  if (housingCostPercent <= 5 || calc.liveFree) {
    verdict = 'Strong Opportunity'
    verdictSub = calc.liveFree ? 'Live for FREE!' : 'Excellent deal - minimal housing cost'
  } else if (housingCostPercent <= 15) {
    verdict = 'Good Opportunity'
    verdictSub = 'Very good - major housing savings'
  } else if (housingCostPercent <= 25) {
    verdict = 'Moderate Opportunity'
    verdictSub = 'Good potential - significant savings'
  } else if (housingCostPercent <= 40) {
    verdict = 'Marginal Opportunity'
    verdictSub = 'Possible deal - some savings'
  } else if (housingCostPercent <= 60) {
    verdict = 'Unlikely Opportunity'
    verdictSub = 'Limited savings vs renting'
  } else {
    verdict = 'Pass'
    verdictSub = 'Not recommended - little benefit'
  }

  const targets = [
    {
      label: 'Housing Cost',
      actual: calc.yourHousingCost,
      target: 0,
      unit: '$',
      met: calc.yourHousingCost <= 0,
    },
    {
      label: 'Savings vs Rent',
      actual: calc.savingsVsRenting,
      target: 500,
      unit: '$',
      met: calc.savingsVsRenting >= 500,
    },
    { label: 'CoC Return', actual: calc.cocReturn, target: 8, unit: '%', met: calc.cocReturn >= 8 },
    {
      label: 'Move-Out CF',
      actual: calc.fullRentalCashFlow,
      target: 200,
      unit: '$',
      met: calc.fullRentalCashFlow >= 200,
    },
  ]
      ...
      <div className="w-full bg-white border-b border-slate-200">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 sm:gap-3">
            <div
              className={`rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 ${calc.liveFree ? 'bg-emerald-500/15' : calc.yourHousingCost < marketRent ? 'bg-blue-500/10' : 'bg-red-500/10'}`}
            >
              ...
              <div
                className={`text-xs sm:text-sm lg:text-base font-bold tabular-nums truncate ${calc.liveFree ? 'text-emerald-600' : calc.yourHousingCost < marketRent ? 'text-blue-600' : 'text-red-500'}`}
              >
                {fmt.currencyCompact(calc.yourHousingCost)}/mo
              </div>
            </div>
            <div className="rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 bg-emerald-500/10">
              ...
              <div className="text-xs sm:text-sm lg:text-base font-bold text-emerald-600 tabular-nums truncate">
                {fmt.currencyCompact(calc.grossRentalIncome)}/mo
              </div>
            </div>
            ...
            <div
              className={`rounded-lg sm:rounded-xl p-2 sm:p-3 lg:p-4 text-center min-w-0 ${calc.fullRentalCashFlow >= 0 ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}
            >
              <div className="text-[8px] sm:text-[9px] lg:text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-0.5 sm:mb-1 truncate">
                Move-Out CF
              </div>
              <div
                className={`text-xs sm:text-sm lg:text-base font-bold tabular-nums truncate ${calc.fullRentalCashFlow >= 0 ? 'text-emerald-600' : 'text-red-500'}`}
              >
                {fmt.currencyCompact(calc.fullRentalCashFlow)}/mo
              </div>
            </div>
            ...
            <div className="grid grid-cols-[1.4fr,1.2fr] md:grid-cols-[1.5fr,1.2fr] lg:grid-cols-[1.2fr,1fr] gap-4 sm:gap-6 items-start">
              <div className="space-y-3">
                <Section index={0} title="Property & Purchase" iconKey="home">
                  <div className="py-3">
                    ...
                    <select
                      ...
                      className="mt-2 w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                  ...
                  <InputRow
                    ...
                    step={0.5}
                    format="percent"
                    subValue={fmt.currency(calc.downPayment)}
                  />
                  <InputRow
                    label="Closing Costs"
                  ...
                  <div className="mt-3 pt-3">
                    <SummaryBox
                      ...
                      value={fmt.currency(calc.totalCashNeeded)}
                      variant="blue"
                    />
                  </div>
                </Section>
                ...
                <Section index={2} title="Unit Income" iconKey="users">
                  <div className="py-3">
                    <label className="text-sm text-slate-500">You Live In</label>
                    <select
                      value={ownerUnit}
                      onChange={(e) => setOwnerUnit(parseInt(e.target.value))}
                      className="mt-2 w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      {unitRents.map((_, i) => (
                  ...
                  {unitRents.map((rent, i) => (
                    <InputRow
                      key={i}
                      label={`Unit ${i + 1} Rent${i === ownerUnit ? ' (Your Unit)' : ''}`}
                      value={
                        i === 0 ? unit1Rent : i === 1 ? unit2Rent : i === 2 ? unit3Rent : unit4Rent
                      }
                      onChange={
                        i === 0
                          ? setUnit1Rent
                  ...
                  <div className="mt-3 pt-3">
                    <SummaryBox
                      ...
                      value={`${fmt.currency(calc.grossRentalIncome)}/mo`}
                      variant="success"
                    />
                  </div>
                </Section>
                ...
                <Section index={3} title="Expenses" iconKey="expense">
                  ...
                  <div className="mt-3 pt-3">
                    <SummaryBox
                      ...
                      value={`−${fmt.currency(calc.totalExpenses)}`}
                      variant="danger"
                    />
                  </div>
                </Section>
                ...
                <Section index={4} title="Cash Flow" iconKey="cashflow">
                  <DisplayRow
                    label="Rental Income"
                    value={`+${fmt.currency(calc.grossRentalIncome)}`}
                    variant="success"
                  />
                  <DisplayRow
                  ...
                  <DisplayRow
                    label="Mortgage (P&I)"
                    value={`−${fmt.currency(calc.monthlyPayment)}`}
                    variant="danger"
                  />
                  <div className="mt-3 pt-3 space-y-2">
                    <SummaryBox
                      label="Your Housing Cost"
                      value={`${fmt.currency(calc.yourHousingCost)}/mo`}
                      variant={calc.liveFree ? 'success' : 'default'}
                    />
                    <SummaryBox
                      label="If You Move Out"
                      value={`${fmt.currency(calc.fullRentalCashFlow)}/mo`}
                      variant={calc.fullRentalCashFlow >= 0 ? 'success' : 'danger'}
                    />
                  </div>
                </Section>
                ...
                <Section index={5} title="Returns" iconKey="returns">
                  ...
                  <DisplayRow
                    ...
                    value={`+${fmt.currency(calc.savingsVsRenting)}/mo`}
                    variant={calc.savingsVsRenting > 0 ? 'success' : 'danger'}
                  />
                  <DisplayRow
                    label="CoC Return"
                    value={fmt.percent(calc.cocReturn)}
                    variant={calc.cocReturn >= 8 ? 'success' : 'default'}
                  />
                  <DisplayRow
              ...
              <div className="space-y-4 sm:sticky sm:top-28">
                    ...
                    {targets.map((t, i) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {t.unit === '$' ? fmt.currencyCompact(t.actual) : fmt.percent(t.actual)}
                          </span>
                          <span className="text-[10px] text-slate-400">
                            / {t.unit === '$' ? fmt.currencyCompact(t.target) : `${t.target}%`}
                          </span>
                          {t.met ? (
                            <svg
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 p-5">
                  ...
                  <div className="relative h-3 bg-gradient-to-r from-emerald-500 via-amber-400 to-red-500 rounded-full mb-4">
                    <div
                      ...
                      style={{
                        left: `calc(${Math.min(100, Math.max(0, (calc.yourHousingCost / marketRent) * 100))}% - 8px)`,
                      }}
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div className="flex justify-between items-center py-2 px-3 bg-slate-50 rounded-lg">
                      ...
                      <span className="font-semibold text-slate-800">
                        {fmt.currencyCompact(marketRent)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center py-2 px-3 bg-slate-50 rounded-lg">
                      <span className="text-slate-500">Your Cost</span>
                      <span
                        className={`font-semibold ${calc.liveFree ? 'text-emerald-600' : 'text-blue-600'}`}
                      >
                        {fmt.currencyCompact(calc.yourHousingCost)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center py-2 px-3 bg-slate-50 rounded-lg col-span-2">
                      <span className="text-slate-500">Monthly Savings</span>
                      <span
                        className={`font-semibold ${calc.savingsVsRenting > 0 ? 'text-emerald-600' : 'text-red-500'}`}
                      >
                        +{fmt.currencyCompact(calc.savingsVsRenting)}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200/60 p-5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div
                      className={`py-3 px-4 rounded-lg ${calc.liveFree ? 'bg-emerald-50 border border-emerald-200' : 'bg-blue-50 border border-blue-200'}`}
                    >
                      <div className="text-xs text-slate-500 mb-1">Housing Cost</div>
                      <div
                        className={`text-lg font-bold ${calc.liveFree ? 'text-emerald-600' : 'text-blue-600'}`}
                      >
                        {fmt.currencyCompact(calc.yourHousingCost)}
                      </div>
                    </div>
                    <div className="py-3 px-4 rounded-lg bg-slate-50">
                      <div className="text-xs text-slate-500 mb-1">Rental Income</div>
                      <div className="text-lg font-bold text-emerald-600">
                        {fmt.currencyCompact(calc.grossRentalIncome)}
                      </div>
                    </div>
                    <div className="py-3 px-4 rounded-lg bg-slate-50">
                      <div className="text-xs text-slate-500 mb-1">CoC Return</div>
                      <div
                        className={`text-lg font-bold ${calc.cocReturn >= 8 ? 'text-blue-600' : 'text-slate-800'}`}
                      >
                        {fmt.percent(calc.cocReturn)}
                      </div>
                    </div>
                    <div className="py-3 px-4 rounded-lg bg-slate-50">
                      <div className="text-xs text-slate-500 mb-1">Move-Out CF</div>
                      <div
                        className={`text-lg font-bold ${calc.fullRentalCashFlow >= 0 ? 'text-emerald-600' : 'text-red-500'}`}
                      >
                        {fmt.currencyCompact(calc.fullRentalCashFlow)}
                      </div>
                    </div>
                  </div>
                </div>
