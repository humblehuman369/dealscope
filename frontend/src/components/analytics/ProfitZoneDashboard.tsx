...
function MetricBox({
  ...
  value,
  sublabel,
  valueColorClass = 'text-[var(--status-positive)]',
}: MetricBoxProps) {
  return (
    <div className="bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-center shadow-sm">
      ...
      <div className={`text-base font-bold ${valueColorClass} leading-tight`}>{value}</div>
      <div className="text-[8px] text-[var(--text-label)]">{sublabel}</div>
    </div>
  )
}
...
function ProfitZoneVisualizer({
  projectedProfit,
  incomeValue,
  listPrice,
}: ProfitZoneVisualizerProps) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="flex items-center gap-1 mb-2">
        <span
          className={`text-sm ${isProfit ? 'text-[var(--status-positive)]' : 'text-[var(--status-negative)]'}`}
        >
          {isProfit ? '↗' : '↘'}
        </span>
        <span
          className={`text-[11px] font-bold tracking-wide ${isProfit ? 'text-[var(--status-positive)]' : 'text-[var(--status-negative)]'}`}
        >
      ...
      <div className="relative w-24 h-44 mx-auto">
        <div
          ...
          style={{
            background:
              'linear-gradient(to bottom, var(--status-positive) 0%, var(--color-green-dim) 100%)',
          }}
        >
          {/* Profit Value Badge */}
          <div
            ...
            style={{
              top: `${Math.max(10, 100 - profitPosition)}%`,
...
const TIP_COLORS: Record<string, { border: string; text: string }> = {
  ...
  action: {
    border: 'border-l-[var(--accent-brand-blue)]',
    text: 'text-[var(--accent-brand-blue)]',
  },
}
    ...
    <div className="bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-xl p-4 shadow-sm">
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] gap-4 items-start min-h-[280px]">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          <MetricBox label="BUY PRICE" value={formatCompactCurrency(buyPrice)} sublabel="Target" />
          <MetricBox
            label="CASH NEEDED"
...
export function generateProfitZoneTips(
  metrics: ProfitZoneMetrics,
  projectedProfit: number,
): ProfitZoneTip[] {
