...
export function HeroMetric({ data }: HeroMetricProps) {
  ...
  return (
    <div className={`${colors.bg} border ${colors.border} rounded-2xl p-4 text-center mb-3.5`}>
      ...
      {/* Subtitle */}
      {data.subtitle && <div className="text-[0.72rem] text-white/50 mt-0.5">{data.subtitle}</div>}

      {/* Badge */}
      {data.badge && (
        <div
          className={`inline-block px-2.5 py-1 ${colors.badge} text-[0.65rem] font-bold rounded-lg mt-2`}
        >
          {data.badge}
        </div>
      )}
 ...
 */
export function create10YearROIHero(roiPercent: number, initialInvestment: number): HeroMetricData {
  const formatCurrency = (value: number) =>
    new Intl.NumberFormat('en-US', {
      style: 'currency',
      currency: 'USD',
 ...
 */
export function createHousingCostHero(effectiveCost: number, marketRent: number): HeroMetricData {
  const formatCurrency = (value: number) =>
    new Intl.NumberFormat('en-US', {
      style: 'currency',
      currency: 'USD',
