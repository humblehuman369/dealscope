...
export function VerdictIQPage({
  ...
  const verdict = useMemo<VerdictData>(() => {
    const dealGapPercent =
      ((pricing.incomeValue - pricing.marketEstimate) / pricing.marketEstimate) * 100
    ...
    return {
      ...
      subtitle:
        score >= 80
          ? 'Deal Gap likely achievable'
          : score >= 60
            ? 'Moderate negotiation needed'
  ...
  const PriceCard = ({
    ...
    >
      <div
        className={`text-[10px] font-bold uppercase tracking-wide mb-1 flex items-center justify-center gap-1 ${
      ...
      <div
        className={`text-base font-bold mb-1 ${recommended ? 'text-[var(--accent-sky)]' : 'text-[var(--text-heading)]'}`}
      >
        {formatPrice(value)}
      </div>
  ...
  const FactorRow = ({ icon, label, value, positive }: OpportunityFactor) => {
    ...
    return (
      <div className="flex justify-between items-center py-2">
        ...
        <span
          className={`text-[13px] font-semibold ${positive ? 'text-[var(--accent-sky)]' : 'text-[var(--text-label)]'}`}
        >
          {value}
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-36">
        ...
        <div className="bg-[var(--surface-card)] p-5 px-6 border-b border-[var(--border-subtle)] flex items-center gap-4">
          <div
            ...
            style={{
              background:
                'linear-gradient(135deg, var(--surface-section) 0%, var(--surface-card) 100%)',
            }}
          >
            <span className="text-[28px] font-extrabold text-[var(--accent-sky)]">
              {verdict.score}
            </span>
          </div>
          <div className="flex-1">
            ...
            <div className="flex gap-3 flex-wrap">
              <span className="flex items-center gap-1 text-[11px] text-[var(--text-secondary)]">
                ...
                Gap:{' '}
                <span className="font-semibold text-[var(--accent-sky)]">{verdict.dealGap}</span>
              </span>
              <span className="flex items-center gap-1 text-[11px] text-[var(--text-secondary)]">
                <svg
                  className="w-3 h-3 text-[var(--text-label)]"
                ...
                Motivation:{' '}
                <span className="font-semibold text-[var(--accent-sky)]">{verdict.motivation}</span>
              </span>
            </div>
        ...
        <div className="bg-[var(--surface-card)] p-4 px-6 border-b border-[var(--border-subtle)]">
          <div className="flex justify-between items-start mb-1">
            <div>
              <div className="text-[15px] font-bold text-[var(--text-heading)]">
                YOUR INVESTMENT ANALYSIS
              </div>
              <div className="text-xs text-[var(--text-secondary)]">
                Based on YOUR financing terms (20% down, 6.0%)
              </div>
            </div>
          ...
          {isOffMarket && (
            <div className="flex items-start gap-2.5 p-3 bg-[var(--surface-section)] rounded-lg mb-4 border-l-[3px] border-l-[var(--accent-sky)]">
              <svg
                className="w-[18px] h-[18px] text-[var(--accent-sky)] flex-shrink-0 mt-0.5"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <div className="text-xs text-[var(--text-body)] leading-relaxed">
                <strong className="text-[var(--accent-sky)]">Off-Market Property:</strong> No asking
                price available. Using Market Estimate of {formatPrice(pricing.marketEstimate)} for
          ...
          <div className="flex items-center justify-between mb-3">
            <span className="text-[13px] text-[var(--text-secondary)]">
              Three ways to approach this deal:
            </span>
            <button
        ...
        <div className="bg-[var(--surface-card)] p-4 px-6 border-b border-[var(--border-subtle)]">
          ...
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-2 text-sm font-semibold text-[var(--text-heading)]">
              <svg
                className="w-[18px] h-[18px] text-[var(--text-secondary)]"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
          ...
          <div className="bg-[var(--surface-section)] rounded-lg p-3 mb-4">
            <div className="flex justify-between items-center py-1.5">
              ...
              <span className="text-[13px] font-semibold text-[var(--text-heading)]">
                {formatPrice(pricing.marketEstimate)}
              </span>
            </div>
            <div className="flex justify-between items-center py-1.5">
              <span className="text-[13px] text-[var(--text-secondary)]">Your Target</span>
              <span className="text-[13px] font-semibold text-[var(--accent-sky)]">
                {formatPrice(pricing.incomeValue)}
              </span>
            </div>
            <div className="flex justify-between items-center py-1.5">
              <span className="text-[13px] text-[var(--text-secondary)]">Discount needed</span>
              <span className="text-[13px] font-semibold text-[var(--accent-sky)]">
                {formatPrice(pricing.discountNeeded)}
              </span>
            </div>
          </div>
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-sm font-semibold text-[var(--accent-sky)]">
          ...
          <div
            ...
            style={{
              background:
                'linear-gradient(135deg, var(--surface-section) 0%, var(--surface-elevated) 100%)',
            }}
          >
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-base font-bold text-[var(--accent-sky)]">
                {sellerMotivation.suggestedOffer}
              </span>
            </div>
          </div>
        </div>
