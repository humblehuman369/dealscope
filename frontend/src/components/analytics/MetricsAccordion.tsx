...
export function MetricsAccordion({
  ...
  return (
    <div
      className={`rounded-xl mb-3 overflow-hidden ${isDark ? 'bg-[#0F1D32]' : 'bg-[var(--surface-card)]'}`}
    >
      {/* Header */}
      <button
      ...
      >
        <div className="flex items-center gap-3">
          <div
            className={`w-9 h-9 rounded-lg flex items-center justify-center ${isDark ? 'bg-[var(--accent-sky)]/15' : 'bg-[var(--accent-sky)]/10'}`}
          >
            {icon}
          </div>
          <div className="text-left">
            <span
              className={`text-[15px] font-semibold ${isDark ? 'text-white' : 'text-[var(--text-heading)]'}`}
            >
              {title}
            </span>
            {subtitle && (
              <span
                className={`block text-[11px] mt-0.5 ${isDark ? 'text-white/50' : 'text-[var(--text-muted)]'}`}
              >
                {subtitle}
              </span>
            )}
          ...
          {metrics.map((metric, index) => (
            ...
            >
              <div className="flex-1">
                <span
                  className={`text-sm font-medium ${isDark ? 'text-white' : 'text-[var(--text-heading)]'}`}
                >
                  {metric.label}
                </span>
                {metric.sublabel && (
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`text-sm font-bold ${isDark ? 'text-white' : 'text-[var(--text-heading)]'}`}
                >
                  {metric.value}
                </span>
                <span
                  className={`px-2 py-1 rounded text-[10px] font-bold tracking-wide ${getGradeStyle(metric.grade)}`}
                >
