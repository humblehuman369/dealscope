...
function getDealZone(
  ...
  const buyBelowIncomeValuePct =
    ((incomeValue - buyPrice) / incomeValue) * 100;

  if (buyPrice > incomeValue) return { zone: 'Loss Zone', motivation: 'Low' };
  if (buyBelowIncomeValuePct < 2) return { zone: 'High Risk', motivation: 'Moderate' };
  if (buyBelowIncomeValuePct < 5) return { zone: 'Income Value / Negotiate', motivation: 'Moderate' };
  if (buyBelowIncomeValuePct < 12) return { zone: 'Profit Zone', motivation: 'High' };
...
function computeDealGapData(
  ...
  const buyVsIncomeValuePercent =
    incomeValue > 0 ? (buyPrice / incomeValue - 1) * 100 : 0;
  const listVsIncomeValuePercent =
    incomeValue > 0 ? (listPrice / incomeValue - 1) * 100 : 0;
...
export function useDealGap(input: UseDealGapInput): UseDealGapResult {
  ...
  const data = useMemo(
    () => computeDealGapData(listPrice, incomeValue, buyPrice, dealScoreFromApi, dealGradeFromApi),
 ...
 * 0 = above Income Value (loss zone), 50 = at Income Value, 100 = deep value
 */
